...
namespace IO
{
	...
	public:
		...
		NDnetUnstructuredMeshWriterT(M *m,
									 ...
									 int defaultNThreads = glb::num_omp_threads) : mesh(m),
																				   options(opt),
																				   useVArr(false),
																				   useSArr(false),
																				   useFacetArr(false),
		...
		void write(BW *bWriter)
		{
			...
			if (nData)
			{
				...
				if (options & NDNET_NoSimplexData)
					nData = 0;
				type = NDIM;
				...
				else // if (NDIM!=2)
				{
					if (nThreads == 1)
					{
						...
						for (auto it = mesh->simplexBegin(); it != it_end; ++it)
						{
							...
							int nOwned = (*it)->getOwnedLocalSegmentHandles(handles, options & NDNET_WithGhosts);
		  ...
		  #pragma omp parallel for num_threads(nThreads)
				...
				*/
				...
				for (long i = 0; i < vIndexArr.size(); ++i)
					if (vIndexArr[i] != std::numeric_limits<LocalIndex>::max())
						vIndexArr[i] = count++;

...
namespace IO
{
      ...
      else
  {
    ...
    static int IsNDnetwork(const char *filename)
    {
      ...
      {
        glb::console->printFlush<LOG_WARNING>("File %s does not exist.\n", filename);
        // fprintf(stderr,"File %s does not exist.\n",filename);
        return 0;
      }

      myIO::fread(&i, sizeof(int), 1, f, swap);
      if (i != NDNETWORK_DATA_STR_SIZE)
        swap = 1 - swap;
    ...
    int readHeader(const char *filename, int verbose = 1)
    {
      ...
      {
        glb::console->printFlush<LOG_WARNING>("File %s is not a valid NDnetwork.\n", filename);
        return -1;
      }
      ...
      if (dummy != NDNETWORK_DATA_STR_SIZE)
        swap = 1 - swap;
      myIO::fread(tag, sizeof(char), NDNETWORK_DATA_STR_SIZE, f, swap);
      myIO::fread(&dummy, sizeof(int), 1, f, swap);
      ...
      if (verbose > 0)
        glb::console->printFlush<LOG_STD>("Loading %dD network from file \"%s\" ...", ndims, filename);
    ...
    int read(const char *filename, int verbose = 1)
    {
      ...
      for (i = 0; i < 1 + ndims; i++)
        if (haveFFlags[i])
        {
          ...
          if (j)
            f_flag[i] = (unsigned char *)malloc(sizeof(unsigned char) * (size_t)nfaces[i]);
          myIO::fread(&dummy, sizeof(int), 1, f, swap);
          if (j)
            myIO::fread(f_flag[i], sizeof(unsigned char), (size_t)nfaces[i], f, swap);
          myIO::fread(&dummy, sizeof(int), 1, f, swap);
        }
    ...
    int write(const char *filename, int verbose = 0)
    {
      ...
      if (verbose > 1)
        printf("Saving %dD network to file %s ...", ndims, filename);
      fflush(0);
      ...
      for (i = 0; i < 1 + ndims; i++)
        if (haveFFlags[i])
        {
          ...
          fwrite(&j, sizeof(int), 1, f);
          if (j)
            fwrite(f_flag[i], sizeof(unsigned char), (size_t)nfaces[i], f);
          fwrite(&j, sizeof(int), 1, f);
        }
  ...
  private:
    ...
    void create(int ndims_, int ndims_net_, T nCells[])
    {
      ...
      for (k = 1; k <= ndims_net; k++)
        haveVertexFromFace[k] = (nCells[k] > 0) ? 1 : 0;

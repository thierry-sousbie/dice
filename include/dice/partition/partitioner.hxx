...
#pragma omp parallel for
        ...
        for (unsigned long i = 2; i <= nSimplices; i++)
          xadj[i] += xadj[i - 1];
...
#pragma omp parallel for
    ...
    glb::console->printFlush<LOG_PEDANTIC>("(%s) ", PartitionTypeSelect().getString(p.type).c_str());
    ...
    */
    ...
    for (unsigned long i = 0; i < nPart; i++) // sender
    {
      for (unsigned long j = 0; j < nPart; j++) // receiver
      {
        ...
        if (nExchg == 0)
          continue; // nobody does anything
        if ((i != rank) && (j != rank))
          continue; // I do nothing ...

        if (i == j)
        {
          Index *snd = &sendBuf[j][0];

          glb::console->printNewLine<LOG_DEBUG>("Sending %ld elements to myself\n", nExchg);
        ...
        else if (i == rank) // I send
        {
          ...
          glb::console->printNewLine<LOG_DEBUG>("Sending %ld element to %ld\n", nExchg, j);

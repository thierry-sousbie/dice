  ...
  */
  void build(const Params &p, bool forceAlternate_ = false)
  {
    ...
    for (int i = 0; i < NDIM; i++)
      xmax[i] = x0[i] + delta[i];
    for (int i = 0; i < NDIM; i++)
      deltaHalf[i] = delta[i] / 2;
    for (int i = 0; i < NDIM; i++)
      stepSize[i] = delta[i] / dims[i];
...
protected:
    ...
    bool contains(const MyType &other) const
    {
      ...
      while ((j < other.nVertices) && (i < nVertices))
      {
        if (vertices[i] < other.vertices[j])
          i++;
        else if (other.vertices[j] < vertices[i])
          j++;
        else
        {
          nFound++;
  ...
  */
  ...
  bool checkAlternate()
  {
    ...
    for (unsigned long j = 0; j < cells.size(); j++)
    {
      ...
      if (config == 0)
        continue;
      if (getNeighborCellPtr(s, config) == NULL)
        return true;
    }
  ...
  long simplexConfigToIndex(SimplexConfig &config, const T refCoords[NDIM]) const
  {
    ...
    for (int i = 0; i < NDIM; i++)
      index += coord[i] * cellStride[type][i];
  ...
  */
  ...
  void build(TesselationType t = TesselationTypeV::ANY)
  {
    ...
    if (t != TesselationTypeV::USER_DEFINED)
      actualType = setSimplices(t);

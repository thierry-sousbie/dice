...
struct SimplicialGridParamsT
{
			   ...
			   int which = 0)
	{
		...
		{
			x0[i] = manager.get("x0", parserCategory, x0[i], i + which * NDIM_W,
								reader, PM::FILE_FIRST,
								"Initial coordinates of the mesh lower left corner");

			delta[i] = manager.get("delta", parserCategory, delta[i], i + which * NDIM_W,
								   reader, PM::FILE_FIRST,
			...
			if (i < NDIM)
			{
				...
				resolution[i] = manager.get("resolution", parserCategory, resolution[i], i + which * NDIM,
											reader, PM::FILE_FIRST,
											"Resolution of the initial mesh (in pixels before tesselation)");
			}
		}

		std::string initTesselationTypeStr = manager.template get<std::string>("initTesselationType", parserCategory,
																			   TesselationTypeSelect().getString(t, true), which,
																			   reader, PM::FILE_FIRST,
																			   TesselationTypeSelect().getAllString("Topology of the initial grid simplicial tesselation (%s)"));
			   ...
			   int which = 0)
	{
		for (int i = 0; i < NDIM_W; i++)
		{
			x0[i] = parser.get("x0", parserCategory, x0[i], i + which * NDIM_W,
							   "Initial coordinates of the mesh lower left corner");

			delta[i] = parser.get("delta", parserCategory, delta[i], i + which * NDIM_W,
								  "Size of the initial mesh");

			if (i < NDIM)
				resolution[i] = parser.get("resolution", parserCategory, resolution[i], i + which * NDIM,
										   "Resolution of the initial mesh (in pixels before tesselation)");
		}

		std::string initTesselationTypeStr = parser.template get<std::string>("initTesselationType", parserCategory,
																			  TesselationTypeSelect().getString(t, true), which,
																			  TesselationTypeSelect().getAllString("Topology of the initial grid simplicial tesselation (%s)"));
		t = TesselationTypeSelect().getVal(initTesselationTypeStr, true);
	}

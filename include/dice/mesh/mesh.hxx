...
public:
	...
	void build(const IST *implicitTesselation, const Params &meshParams)
	{
		...
		glb::console->printNewLine<LOG_STD>("Creating initial mesh from implicit tesselation:\n");
		glb::console->indent();
		/*
		...
		*/
		...
		if (mpiCom->size() > 1)
		{
			...
			if (ncut >= 0)
				glb::console->print<LOG_STD>("done. (%ld cuts)\n", ncut);
			else
				glb::console->print<LOG_STD>("done.\n");
		}
		...
		for (simplexPtr_iterator it = LocalMesh::simplexBegin(); it != its_end; ++it)
		{
			...
			for (int k = 0; k < Simplex::NNEI; ++k)
			{
				...
				if (it->cache.l != nei->cache.l)
				{
					...
					{
						if (j == k)
							continue; // this one is NOT on the interface
		...
		for (ghostSimplexPtr_iterator it = LocalMesh::ghostSimplexBegin(); it != itgs_end; ++it)
		{
			...
			for (int k = 0; k < Simplex::NNEI; ++k)
			{
				...
				if (it->cache.l != nei->cache.l)
				{
					for (int j = 0; j < Simplex::NVERT; ++j)
					{
						if (it->getVertex(j)->isShadowOrGhost())
							continue;
						if (j == k)
							continue; // this one is NOT on the interface
		...
		for (vertexPtr_iterator it = LocalMesh::vertexBegin(); it != itv_end; ++it)
		{
			...
			if (vertexBallArr[lid] != NULL)
			{
				...
				for (unsigned long i = 0; i < vertexBallArr[lid]->size(); ++i)
				{
					...
					// process they belong ...
					if (!cur->isShadowOrGhost())
						dest.insert(cur->cache.l);
				}
					 ...
					 dst_it != dest.end(); ++dst_it)
				{
					...
					int idx = (*dst_it == myRank) ? (newGhostSimplices.size() - 1) : sendRankIndex[*dst_it];
...
#pragma omp parallel for num_threads(nThreads)
		...
		*/
		...
		for (unsigned long i = 0; i < leavesExchange.receiveRank.size(); ++i)
		{
			...
			if (count > 0)
				receivedGhostSimplices[srcIndex].resize(count);
			else
				receivedGhostSimplices[srcIndex].clear();
...
#pragma omp critical
							{ // START OF CRITICAL
								...
								if (it == shadowSimplicesHash.end())
								{
									...
									shadowSimplicesHash.insert(std::make_pair(cur[j].neighbors[k],
																			  static_cast<Simplex *>(shadow)));
								}
								else
									shadow = static_cast<ShadowSimplex *>(it->second);
		...
		for (ghostSimplexPtr_iterator it = LocalMesh::ghostSimplexBegin(); it != itgs_end; ++it)
		{
			...
			for (int j = 0; j < Simplex::NVERT; ++j)
			{
				...
				if (v->isTagged2())
				{
					...
					if (ghost_it == ghostVertexHash.end())
					{
						...
						gv = static_cast<GhostVertex *>(ghost_it->second);
					it->setVertex(j, gv);
			...
			if (gvCount < NDIM)
			{
				...
				for (int i = 0; i < Simplex::NNEI; ++i)
					if (it->getNeighbor(i)->isShadow())
						failed = true;
				if (failed)
				{
					glb::console->print<LOG_STD_ALL>("WARNING: This ghost simplex may not have a shadow neighbor !\n");
					static_cast<Simplex *>(*it)->template print<LOG_STD_ALL>("Ghost:");
		...
		for (int i = 0; i < requestsCount; i++)
		{
			...
			for (unsigned long j = 0; j < received.size(); ++j)
			{
				...
				if (it == simplicesHash.end())
				{
					...
					glb::console->print<LOG_ERROR>("Could not retrieve ghost simplex origin.\n");
					exit(-1);
				}
...
#pragma omp parallel for num_threads(nThreads)
		...
		for (unsigned long i = 0; i < ghostExchange.receiveRank.size(); ++i)
		{
			...
			{
				GhostSimplex *ref = ghostExchange.receive[src][sendSSFGQ[i][j].getBaseCellIndex()];
						...
						if (it == ghostVertexHash.end()) // new shadow vertex
						{
							...
							if (it == shadowVertexHash.end())
							{
								if (k != receivedSSFGR[i][j].vertexIndex)
								{
									...
									static_cast<Simplex *>(ref)->template print<LOG_STD_ALL>(tmp);
									receivedSSFGR[i][j].template print<LOG_STD_ALL>();
									if (shadow != NULL)
										shadow->template print<LOG_STD_ALL>();

									PRINT_SRC_INFO(LOG_ERROR);
									glb::console->print<LOG_ERROR>("There are inconsistencies in the global identities, vertex (%d,%d) not found in ghostVertexHash\n",
								...
								v = static_cast<ShadowVertex *>(it->second);
						}
						else
							v = static_cast<ShadowVertex *>(it->second);
...
#pragma omp parallel for num_threads(nThreads)
		...
		*/
		...
		if (glb::debug)
			checkConsistencyAndReport<LOG_ERROR>("repart");
		/*
		const simplexPtr_iterator it_end=LocalMesh::simplexEnd();
		for (simplexPtr_iterator it=LocalMesh::simplexBegin();it!=it_end;++it)
		  {
				...
				do
				{
					...
					else
					{
						...
						else
						{
							...
							if (curGid != otherGid)
							{
								bool cmp = LocalMesh::compareSegmentHandlesLess(it->second, other->second, true);
								/*
...
#pragma omp parallel for num_threads(nThreads)
			...
			for (unsigned long i = 0; i < toRefine.size(); ++i)
			{
				...
				if (s->cache.ptr == NULL)
					continue;
				if (s->isShadowOrGhost())
					nGhostRefined++;
				if (i != toRefineCount)
					std::swap(toRefine[toRefineCount], toRefine[i]);
...
#pragma omp parallel for num_threads(nThreads)
	  ...
	  #pragma omp parallel for num_threads(nThreads)
			for (int th=0; th<nThreads; th++)
		  {
			...
			*/
			...
			if (glb::debug)
				checkConsistencyAndReport<LOG_ERROR>("refine");

			if (nRemoteConflicts > 0)
			{
				if (nRefined)
					glb::console->print<LOG_INFO>("done. (%ld segments refined, %ld remote conflicts)\n",
		...
		if ((!glb::console->willPrint<LOG_INFO>()) && (glb::console->willPrint<LOG_STD>()))
			...
			glb::console->print<LOG_INFO>("A total of %ld/%ld segments were Locally/Globally refined in %.2fs.\n",
										  nRefinedLocalTotal, nRefinedTotal, timeSpent);
		else
			glb::console->print<LOG_INFO>("No refinement needed (checking took %.2fs).\n", timeSpent);
		...
		if (glb::debugMeshRefine)
		{
			...
			if (nRefined == 0)
			{
				...
				nRefinedLocalTotal += refine(solver, nThreads);
		}
	...
	*/
	...
	{
		// std::vector<Simplex*> sList;
		const long rBufferSize = Vertex::template getSimplexRefineBufferSize<MyType, SegmentHandle>();
		...
		{
			// Update coordinates and data
			Vertex::Data::onRefineVertex(this, handle, &newVertex, &sList[0], sList.size(), buffer);
		}
		else
		{
			// Only the coordinates of the vertex are updated in that case
		...
		{
			splitSimplices[0][n].splitSegment(handle, NULL, &newVertex, &splitSimplices[1][n]);
			...
			if (updateSimplexData)
			{
				...
				{
					// Update only data with index UpdateDataIndex
					Simplex::Data::template refineSimplexData<UpdateDataIndex>(this, handle, &newVertex, &s0, &s1, 1, curBuffer);
				}
				else
				{
					// update all data
					Simplex::Data::onRefineSimplices(this, handle, &newVertex, &s0, &s1, 1, curBuffer);
				}
			}
			curBuffer += rBufferSize;
		}
									  ...
									  bool updateSimplexData = true) const
	{
		...
		splitSimplices[0].splitSegment(handle, NULL, &newVertex, &splitSimplices[1]);
...
#pragma omp critical
		...
		if (removedVerticesLID.capacity() < 2 * candidates.size())
		{
			...
			if (NDIM == 1)
				removedSimplicesLID.reserve(candidates.size());
			if (NDIM == 2)
				removedSimplicesLID.reserve(candidates.size() * 2);
			else
				removedSimplicesLID.reserve(candidates.size() * 5); // wild guess ;)
		}
...
#pragma omp critical
						{
							for (unsigned long n = 0; n < s2.size(); ++n)
							{
								...
								Simplex::Data::template onCoarsenSimplex<MyType, Simplex, Vertex>(this, keep, keepVId, rmIndex, rm, rmVId, keepIndex,
...
#pragma omp parallel for
		...
		#pragma omp parallel for
			...
			*/
			...
			for (unsigned long i = 0; i < ghostExchange.receiveRank.size(); ++i)
				receiveGhostRankIndex.insert(std::make_pair(ghostExchange.receiveRank[i], i));
			// receiveGhostRankIndex[ghostExchange.receiveRank[i]] = i;
			std::map<int, int> receiveShadowRankIndex;
			for (unsigned long i = 0; i < shadowExchange.receiveRank.size(); ++i)
				receiveShadowRankIndex.insert(std::make_pair(shadowExchange.receiveRank[i], i));

			std::map<int, int> sendGhostRankIndex;
			for (unsigned long i = 0; i < ghostExchange.sendRank.size(); ++i)
				sendGhostRankIndex.insert(std::make_pair(ghostExchange.sendRank[i], i));
			std::map<int, int> sendShadowRankIndex;
			for (unsigned long i = 0; i < shadowExchange.sendRank.size(); ++i)
				sendShadowRankIndex.insert(std::make_pair(shadowExchange.sendRank[i], i));
			...
			for (unsigned long i = 0; i < ghostExchange.receiveRank.size(); ++i)
			{
				...
				if (count > 0)
					receivedGhosts[srcIndex].resize(count);
				else
					receivedGhosts[srcIndex].clear();
				...
				for (unsigned long j = 0; j < receivedGhosts[srcIndex].size(); ++j)
				{
					...
					if (receivedGhosts[srcIndex][j].type == 0)
					{
						...
						Simplex *keep_ = rm_->getNeighbor(receivedGhosts[srcIndex][j].partnerIndex);
						if (!keep_->isShadow())
							continue;
						...
						if (newV->isShadow())
						{
							...
							if (it == ShadowVertex2GhostVertexMap.end())
							{
								...
								newGV = static_cast<Vertex *>(it->second);
						}
						...
						else
						{
							...
							if (!val.isEmpty())
							{
								...
								if ((nei != NULL) && (!nei->isShadowOrGhost()))
									shadowExchange.send[ghostExchange.sendRank[srcIndex]].push_back(nei);
								else if (nei != NULL)
								{
...
#pragma omp parallel for
			...
			for (unsigned long i = 0; i < ghostExchange.receiveRank.size(); ++i)
			{
				...
				if (count > 0)
					receivedNewShadowSimplices[srcIndex].resize(count);
				else
					receivedNewShadowSimplices[srcIndex].clear();
						...
						*/
						...
						for (int k = 0; k < Simplex::NVERT; ++k)
						{
							...
							else
							{
								...
								{
									PRINT_SRC_INFO(LOG_ERROR);
									glb::console->print<LOG_ERROR>("Error while coarsening, I messed up with the global IDs :( \n");
									cur->template print<LOG_ERROR>();
									ss->template print<LOG_ERROR>();
									glb::console->print<LOG_ERROR>("When looking for vertex with GID=(%ld,%ld) at baseIndex = %d.\n",
				...
				for (unsigned long i = 0; i < transferedReceiveQueries.size(); ++i)
				{
					...
					if (shadowExchange.receive[rank].size() == 0)
					{
						...
						for (unsigned long j = 0; j < shadowExchange.receiveRank.size(); ++j)
							if (shadowExchange.receiveRank[j] == rank)
								found = 1;
						if (!found)
						{
							// glb::console->print<LOG_STD_ALL>("ADDING NEW RANK !!!!!!\n");
							shadowExchange.receiveRank.push_back(rank);
							receiveShadowRankIndex.insert(std::make_pair(rank, shadowExchange.receiveRank.size() - 1));
						}
					}
			...
			{
				int count = 0;
				int source = mpiCom->template ProbeCount<GlobalIdentityValue>(count, mpiTagsStart_coarsen + 5);
				...
				if (query.size() > 0)
				{
					...
					for (long j = 0; j < nQueries; ++j)
					{
						...
						if (it == sendShadowRankIndex.end())
						{
							...
							it = sendShadowRankIndex.insert(std::make_pair(gid.rank(),
			...
			for (long i = 0; i < shadowExchange.receiveRank.size(); ++i)
			{
				...
				typename std::vector<ShadowSimplex *>::iterator it =
					std::unique(shadowExchange.receive[rank].begin(),
								shadowExchange.receive[rank].end());
				shadowExchange.receive[rank].resize(std::distance(shadowExchange.receive[rank].begin(), it));
			}
			...
			for (long i = 0; i < shadowExchange.sendRank.size(); ++i)
			{
				...
				typename std::vector<Simplex *>::iterator it =
					std::unique(shadowExchange.send[rank].begin(),
								shadowExchange.send[rank].end());
				shadowExchange.send[rank].resize(std::distance(shadowExchange.send[rank].begin(), it));
			}
		...
		if (nCoarsened > 0)
		{
			...
			for (simplexPtr_iterator it = LocalMesh::simplexBegin(); it != its_end; ++it)
			{
				if (it->getLocalIndex() >= NSmax)
				{
					...
					if ((++rms_it) == removedSimplicesLID.end())
						break;
				}
			}
			...
			for (vertexPtr_iterator it = LocalMesh::vertexBegin(); it != itv_end; ++it)
			{
				if (it->getLocalIndex() >= NVmax)
				{
					...
					if ((++rmv_it) == removedVerticesLID.end())
						break;
				}
			}
		} // nCoarsened
		...
		if (nParts > 1)
		{
			...
			{
				glb::console->print<LOG_PEDANTIC_ALL>("INFO: ghostExchange.sendRank has been resized, which means that the number of neighbor processes changed.\n");
				glb::console->print<LOG_PEDANTIC_ALL>("This feature is untested, so if I crash, you'll know why ;)");
			...
			{
				glb::console->print<LOG_PEDANTIC_ALL>("INFO: ghostExchange.receiveRank has been resizes, which means that the number of neighbor processes changed.\n");
				glb::console->print<LOG_PEDANTIC_ALL>("This feature is untested, so if I crash, you'll know why ;)");
			...
			for (shadowSimplexPtr_iterator it = LocalMesh::shadowSimplexBegin(); it != itss_end; ++it)
			{
				...
				else
				{
					for (int i = 0; i < Simplex::NVERT; ++i)
					{
						...
						{
							GlobalIdentityValue gid = it->getVertex(i)->getGlobalIdentity().get();
		...
		for (unsigned long i = 0; i < shadowExchange.sendRank.size(); ++i)
		{
			...
			if (count > 0)
			{
				...
				for (int j = 0; j < count; ++j)
				{
					...
					if (killedShadows[j] < newIndex)
						newIndex = killedShadows[j];
				}
...
#pragma omp parallel for
		...
		if (glb::debug)
			checkConsistencyAndReport<LOG_ERROR>("coarsen");
...
#pragma omp parallel num_threads(nThreads)
						 ...
						 int nThreads = glb::num_omp_threads)
	{
		// #pragma omp critical
		{
			...
			if (needFullUpdate)
			{
				LocalMesh::getIncidentSimplices(incidentSimplices.simplices,
		...
		return std::pair<const std::vector<Simplex *> &,
						 const std::vector<LocalIndex> &>(incidentSimplices.simplices, incidentSimplices.index);
	}
...
protected:
	...
	void checkRefine_singleSimplex(Solver *solver, S *simplex, std::vector<S *> &cSimplices)
	{
		...
		if (result <= 0)
			simplex->cache.ptr = NULL; // clean up
		else
		{
...
#pragma omp task untied
	...
	template <class S>
	void checkRefine_setSimplicesCache_fromCachedSimplices(S *solver, std::vector<char> &check, int nThreads,
...
#pragma omp for nowait
	...
	template <class S>
	void checkRefine_setSimplicesCache(S *solver, std::vector<char> &check, int nThreads,
									   std::vector<std::vector<Simplex *>> &cSimplices,
									   std::vector<std::vector<GhostSimplex *>> &cGSimplices,
									   bool useCache)
	{
		if (useCache)
			return checkRefine_setSimplicesCache_fromCachedSimplices(solver, check, nThreads, cSimplices, cGSimplices);
...
#pragma omp parallel for num_threads(nThreads)
			...
			for (unsigned long j = 0; j < curSend.size(); j++)
			{
				...
				if (me->isTagged())
				{
					...
					if (MpiExchg_RefineShared::IS_INDEXED) // INDEXED
					{
						...
						toSend[i][j].set(refSimplex, me, other, newVertex, myRank); // NON INDEXED
					// #pragma omp critical
		...
		exchangeInfo.exchangeStruct(toSend, received, mpiType_refineShared.getType(), mpiTagsStart_general + 2);

		// add the new simplices to send in exchangeInfo
#pragma omp parallel for num_threads(nThreads)
		for (unsigned long i = 0; i < exchangeInfo.sendRank.size(); i++)
		{
			exchangeInfo.send[exchangeInfo.sendRank[i]].insert(exchangeInfo.send[exchangeInfo.sendRank[i]].end(),
		...
		for (unsigned long i = 0; i < received.size(); i++)
		{
			...
			for (unsigned long j = 0; j < received[i].size(); j++)
			{
				...
				Simplex *targetSimplex =
					(MpiExchg_RefineShared::IS_INDEXED) ? targetSimplexVec[rcvd->getBaseCellIndex()] : targetSimplexVec[j];
				...
				{ // start omp critical
					...
					if (it == sharedVerticesMap.end())
					{
						...
						sharedVerticesMap.insert(std::make_pair(rcvd->ref, static_cast<Vertex *>(newVertex)));
					}
					else
					{
				...
				if ((v0 == NULL) || (v1 == NULL))
				{
					...
					else
					{
						...
						glb::console->template print<LOG_ERROR>("v0 is NULL (gid=(%ld,%ld))\n",
					...
					else
					{
						...
						glb::console->template print<LOG_ERROR>("v1 is NULL (gid=(%ld,%ld))\n",
																(long)gid.rank(), (long)gid.id());
					}
					PRINT_SRC_INFO(LOG_ERROR);
					glb::console->print<LOG_ERROR>("Vertices global identities seem to be out of sync !\n");
					exit(-1);
				}
	...
	void initFromImplicitTesselation(IST *st, const std::vector<VT> &partition)
	{
		...
		if (sizeof(ULong64) != 8)
		{
			if (sizeof(ULong64) < 8)
			{
				...
				glb::console->print<LOG_WARNING>("Type ULong64 is smaller than expected. (%d bytes, should be %d)\n",
												 sizeof(ULong64), 8);
				glb::console->print<LOG_WARNING>("You should edit file 'types.hxx'.\n");
			}
			else
			{
				glb::console->print<LOG_STD>("WARNING: Type ULong64 is bigger than expected. (%d bytes, should be %d)\n",
											 sizeof(T), 8);
		...
		else
		{
			...
			glb::console->printToBuffer<LOG_PEDANTIC>("Estimated local cells count (node 0): [%ld", nCells[0]);
			for (int i = 1; i <= NDIM; i++)
		...
		glb::console->print<LOG_PEDANTIC>("Expected structure size per volume element: %ld x %ld + %ld x %ld = %ld Bytes.\n",
		...
		if (usePartition)
		{
			...
			glb::console->print<LOG_PEDANTIC>("Estimated ghost/shadow simplices count : %ld/%ld.\n",
		...
		*/
		...
		{
			PRINT_SRC_INFO(LOG_ERROR);
			glb::console->print<LOG_ERROR>("Could not find 'projectedVolume' functor for simplices.\n");
			exit(-1);
		}
			...
			if (usePartition)
			{
				for (long j = 0; j < Simplex::NVERT; j++)
				{
					...
					if (it == vertexTable.end())
					{
						...
						vertexTable.insert(std::make_pair(id, (void *)static_cast<Vertex *>(v[j])));
					}
					else
						v[j] = static_cast<Vertex *>(it->second);
				}
				...
				simplexTable.insert(std::make_pair((VT)index, (void *)static_cast<Simplex *>(simplex)));
			}
			else
			{
				for (long j = 0; j < Simplex::NVERT; j++)
...
#pragma omp critical
				...
				for (unsigned long i = 0; i < cellV.size(); i++)
				{
					...
					if (sit == simplexTable.end())
					{
						...
						if (sit == ghostSimplexTable.end())
						{
							...
							for (int j = 0; j < Simplex::NVERT; j++)
							{
								...
								if (vit == vertexTable.end())
								{
									...
									if (vit == ghostVertexTable.end())
									{
										...
										ghostVertexTable.insert(std::make_pair((unsigned long)vertex[j].id(),
																			   (void *)static_cast<Vertex *>(tmpV)));

										v[j] = tmpV;
									}
									else
										v[j] = static_cast<Vertex *>(vit->second);
								}
								else
									v[j] = static_cast<Vertex *>(vit->second);
							...
							ghost->setLocalIndex(LocalMesh::ghostSimplexPool.getUsedCount() - 1);

							ghost->setGlobalIdentity(GlobalIdentity::EMPTY_RANK, neiId);
							ghost->setGeneration(0, neiId);
							ghostSimplexTable.insert(std::make_pair(neiId,
			...
			if (glb::console->willPrint<LOG_INFO>())
				glb::console->print<LOG_INFO>("done. (+%ld/%ld ghost simplices/vertices)\n",
...
#pragma omp critical
						...
						nei[j] = static_cast<Simplex *>(it->second); // regular local neighbor, nothing special
				}
			}
...
#pragma omp critical
		...
		*/
		...
		if (usePartition)
		{
			...
			if (glb::console->willPrint<LOG_PEDANTIC>())
			{
				...
				for (int i = 1; i <= NDIM; i++)
					glb::console->printToBuffer<LOG_PEDANTIC>(",%ld/%ld",
	...
	std::pair<ShadowSimplex *, Vertex *> findOrCreateShadowSimplex(IST *st,
																   ...
																   VT &shadowVertexTable)
	{
		...
		if (itS == shadowSimplexTable.end())
		{
				...
				if (it == vertexTable.end())
				{
					...
					if (it == ghostVertexTable.end())
					{
						...
						if (it == shadowVertexTable.end())
						{
							...
							shadowVertexTable.insert(std::make_pair((unsigned long)id,
			...
			shadowSimplexTable.insert(std::make_pair(shadowId, (void *)static_cast<Simplex *>(shadow)));
		}
		else
			shadow = static_cast<ShadowSimplex *>(static_cast<Simplex *>(itS->second));

		// find the opposite vertex of the shadow simplex
		int difId = hlp::FindFirstDifference<ITCell, Simplex::NVERT>::find(&shadowVert[0], &neighborVert[0]);
						   ...
						   MpiCellDataExchangeT<Simplex, ES> &exchangeInfo)
	{
		...
		for (int i = 0; i < nParts; i++)
		{
			...
			if (myRank != i)
			{
				...
				for (unsigned long j = 0; j < len; j++, dec += (1 + Simplex::NVERT))
				{
					...
					if (it != simplexTable.end())
					{
						...
						for (int k = 0; k < Simplex::NVERT; k++)
						{
							...
							{
								PRINT_SRC_INFO(LOG_ERROR);
								glb::console->print<LOG_ERROR>("local and remote simplex do not mach.\n");
								exit(-1);
							}
				...
				if (nFound[myRank])
					exchangeInfo.sendRank.push_back(i);
			}
			else
				identityArr.assign(totalLen, 0);
			...
			if (myRank == i)
			{
				...
				for (long j = 0; j < nParts; j++)
					if (nFound[j] > 0)
						exchangeInfo.receiveRank.push_back(j);
			}
		}
		exchangeInfo.updateNCum();
	}
	...
	double updateLoadImbalanceFactor()
	{
		...
		{
			if (globalNLocalCells[NDIM][i] > max)
				max = globalNLocalCells[NDIM][i];

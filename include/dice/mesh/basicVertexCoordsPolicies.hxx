				...
				for (int n = 0; n < 2; ++n)
				{
					...
					for (int j = 0; j < S::NVERT; ++j)
					{
						for (int k = 0; k < NDIM_W; ++k)
							point[k] += geometry->checkCoordConsistency(p[n][j][k], result[k], k);
					}
					...
					if (n == 0)
						vIdBuffer[S::NVERT] = seg->getVertex(0);
				}
						   ...
						   S *const *simplices, int nSimplices, void *buffer)
		{
			...
			for (int i = 0; i < nSimplices; ++i)
			{
				...
				for (int n = 0; n < 2; ++n)
				{
					...
					if (n == 0)
						vIdBuffer[S::NVERT] = simplex->getVertex(segVertId.first);
				}
				...
				for (int j = 0; j < S::NVERT; ++j)
				{
					...
					if (it == vertexMap.end())
					{
						...
						for (int k = 0; k < NDIM_W; ++k)
							contribCoord.push_back(geometry->checkCoordConsistency(cE[k], result[k], k));
					}
					else
					{
						index = it->second;
						...
						for (int k = 0; k < NDIM_W; ++k)
							contribCoord[index * NDIM_W + k] += geometry->checkCoordConsistency(cE[k], result[k], k);
					}
			...
			for (int i = 0; i < nSimplices; ++i)
			{
				...
				for (int j = 0; j < S::NVERT; ++j)
				{
					...
					else
					{
						...
						index *= NDIM_W;
						for (int k = 0; k < NDIM_W; ++k)
							contribCoord[index + k] += geometry->checkCoordConsistency(cE[k], result[k], k);
						   ...
						   S *const *simplices, int nSimplices, void *buffer)
		{
			...
			for (int i = 0; i < nSimplices; ++i)
			{
				...
				for (int j = 0; j < S::NVERT; ++j)
				{
					...
					for (int k = 0; k < NDIM_W; ++k)
						origin[i][k] += geometry->checkCoordConsistency(coords[k], midPoint[k], k);
				}
				for (int j = 0; j < NDIM_W; ++j)
					origin[i][j] /= S::NVERT;
			...
			for (int i = 0; i < nSimplices; ++i)
			{
				...
				*/
				...
				if (sf.getPointsCount() < SurfaceFitter::NCOEFS)
				{
					...
					glb::console->print<LOG_ERROR>("System is under-determined, this case is not implemented yet !\n");
					exit(-1);
				}
			...
			for (int i = 0; i < nSimplices; ++i)
			{
				...
				for (int n = 0; n < 2; ++n)
				{
					...
					for (int j = 0; j < S::NVERT; ++j)
					{
						...
						for (int k = 0; k < NDIM_W; ++k)
							org[k] += geometry->checkCoordConsistency(p[n][j][k], midPoint[k], k);
					}
					for (int j = 0; j < NDIM_W; ++j)
						org[j] /= S::NVERT;
		  ...
		  if (simplices[i]->getLocalIndex()==19296) debug=i;
			...
			*/
			...
			for (int i = 0; i < nSimplices; ++i)
			{
				...
				{
					V *v = simplices[i]->getVertex(j);
					if ((v == seg->getVertex(0)) || (v == seg->getVertex(1)))
						continue;
				...
				for (int n = 0; n < 2; ++n)
				{
					...
					if (i==debug)
				  {
					...
					*/
					...
					for (int j = 0; j < NDIM_W; ++j)
						point[j] = (point[j] + p2[j]) * 0.5;
					fit.projectToSurface(point, newTCoord);

					for (int j = 0; j < NDIM_W; ++j)
						newRefSegT[n][j] += newTCoord[j] + org[j];
			...
			{
				for (int j = 0; j < NDIM_W; ++j)
					newSegT[NDIM_W * i + j] /= nContribs[i];
				mesh->getGeometry()->checkBoundary(&newSegT[NDIM_W * i]);
			}
			for (int i = 0; i < 2; ++i)
			{
				for (int j = 0; j < NDIM_W; ++j)
					newRefSegT[i][j] /= nSimplices;

...
private:
  ...
  void setDataElements()
  {
    ...
    for (unsigned long i=0;i<simplexFunctor.size();++i)
      simplexFunctorMap.
	insert(std::make_pair(simplexFunctor[i]->getName(),simplexFunctor[i]));

    // and vertices
    Vertex::createAllDataElementFunctorPtr(mesh,std::back_inserter(vertexFunctor));
    for (unsigned long i=0;i<vertexFunctor.size();++i)
      vertexFunctorMap.
	insert(std::make_pair(vertexFunctor[i]->getName(),vertexFunctor[i]));
  }
...
public:
  ...
  {
    if (i<vertexFunctor.size()) return vertexFunctor[i];
    return NULL;
  }

  SimplexFunctor *getSimplexFunctor(unsigned int i)
  {
    if (i<simplexFunctor.size()) return simplexFunctor[i];
    return NULL;
  }
  ...
  VertexFunctor *getVertexFunctor(const std::string &str)
  {
    ...
    if (it==vertexFunctorMap.end()) return NULL;
  ...
  SimplexFunctor *getSimplexFunctor(const std::string &str)
  {
    ...
    if (it==simplexFunctorMap.end()) return NULL;
    else return it->second;
  }

...
namespace sharedTreeNodes
{
    ...
    int addChild(MyType *c)
    {
      ...
      else if (isNode())
      {
        ...
        else if (children[1] == NULL)
        {
          ...
          {
            static_cast<Leaf *>(children[0])->setSplitIndex(static_cast<const Node *>(this)->getChildIndex(0));
            static_cast<Leaf *>(children[1])->setSplitIndex(static_cast<const Node *>(this)->getChildIndex(1));
  ...
  protected:
                          ...
                          std::vector<IT> &nodeData)
    {
      ...
      {
        if (index == 0)
          compactTree.push_back(0);
        if ((++index) == sizeof(IT) * 8)
          index = 0;
  ...
  protected:
    ...
    Element *merge(NP &nodePool)
    {
      ...
      else
      {
        ...
        if (other->isLeaf())
        {
          keep->setSplitIndex(static_cast<Node *>(grandParent)->getChildIndex(keep));
          static_cast<Leaf *>(other)->setSplitIndex(static_cast<Node *>(grandParent)->getChildIndex(other));
        }
        else
          keep->haveDirectPartner(false);
      }

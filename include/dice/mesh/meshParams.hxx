...
struct MeshParamsT
{
  ...
  */
  ...
  {
    float version;
    BR::template checkHeaderAndReport<LOG_ERROR, LOG_WARNING, MyType>(glb::console, reader, version, true);
    ...
    reader->read(&tmp);
    initPartitionType = static_cast<PartitionType>(tmp);
  ...
  void parse(BR *reader, PM &manager)
  {
    ...
    allocFactor = manager.get("allocFactor", parserCategory(), allocFactor,
                              reader, PM::PARSER_FIRST,
                              "What fraction of the current number of element to reallocate when a pool is full.");

    initPartitionTolerance = manager.get("initPartitionTolerance", parserCategory(), initPartitionTolerance,
                                         reader, PM::FILE_FIRST,
                                         "Tolerance on the load balance of the initial partition");

    repartTolerance = manager.get("repartTolerance", parserCategory(), repartTolerance,
                                  reader, PM::PARSER_FIRST,
                                  "Tolerance on the load balance after repartitionning");

    repartThreshold = manager.get("repartThreshold", parserCategory(), repartThreshold,
                                  reader, PM::PARSER_FIRST,
    ...
    */
    std::string initPartitionTypeStr = manager.template get<std::string>("initPartitionType", parserCategory(),
                                                                         PartitionTypeSelect().getString(initPartitionType, true),
                                                                         reader, PM::FILE_FIRST,
                                                                         PartitionTypeSelect().getAllString("The method to use for initial partitionning (%s)"));
    initPartitionType = PartitionTypeSelect().getVal(initPartitionTypeStr, true);

    std::string refinePartitionTypeStr = manager.template get<std::string>("refinePartitionType", parserCategory(),
                                                                           RefinePartitionTypeSelect().getString(refinePartitionType, true),
                                                                           reader, PM::PARSER_FIRST,
                                                                           RefinePartitionTypeSelect().getAllString("The method to use for repartitionning (%s)"));
  ...
  void parse(PP &parser)
  {
    ...
    allocFactor = parser.get("allocFactor", parserCategory(), allocFactor,
                             "What fraction of the current number of element to reallocate when a pool is full.");

    initPartitionTolerance = parser.get("initPartitionTolerance", parserCategory(), initPartitionTolerance,
                                        "Tolerance on the load balance of the initial partition");

    repartTolerance = parser.get("repartTolerance", parserCategory(), repartTolerance,
                                 "Tolerance on the load balance after repartitionning");

    repartThreshold = parser.get("repartThreshold", parserCategory(), repartThreshold,
                                 "Inbalance factor that triggers repartitionning");
    /*
    ...
    initTesselationType = TesselationTypeSelect().getVal(initTesselationTypeStr,true);
    */
    std::string initPartitionTypeStr = parser.template get<std::string>("initPartitionType", parserCategory(),
                                                                        PartitionTypeSelect().getString(initPartitionType, true),
                                                                        PartitionTypeSelect().getAllString("The method to use for initial partitionning (%s)"));
    initPartitionType = PartitionTypeSelect().getVal(initPartitionTypeStr, true);

    std::string refinePartitionTypeStr = parser.template get<std::string>("refinePartitionType", parserCategory(),
                                                                          RefinePartitionTypeSelect().getString(refinePartitionType, true),
                                                                          RefinePartitionTypeSelect().getAllString("The method to use for repartitionning (%s)"));
    refinePartitionType = RefinePartitionTypeSelect().getVal(refinePartitionTypeStr, true);
  }

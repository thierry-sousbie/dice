...
protected:
  ...
  LocalMeshT(MpiCommunication *com) : Tree(com),
                                      mpiCom(com),
                                      simplexPool("Simplex"),
                                      vertexPool("Vertex"),
...
#pragma omp parallel for
  ...
  void construct()
  {
    ...
    if (NDIM_W > NDIM)
      cellDataFunctors.template insert<cellDataFunctors::ExtraDimsT>(flags);
    ...
    if (!glb::debug)
      flags |= cellDataFunctors::F_SKIP_ON_FILE_DUMP;
...
#pragma omp parallel for num_threads(nThreads)
  ...
  */
                                   ...
                                   bool dumpAll = false, bool withNeighbors = false)
  {
    ...
    if (mpiCom->size() == 1)
      glb::console->printFlush<LOG_STD>("Dumping local %dD mesh to %dDnetwork file '%s' ... ", NDIM, NDIM_W, fname.c_str());
    else
      glb::console->printFlush<LOG_STD>("Dumping local %dD mesh to %dDnetwork files '%s' (%d files) ... ",
    ...
    if (f == NULL)
    {
      ...
      glb::console->print<LOG_ERROR>("Could not open file '%s' for writing.\n", fname.c_str());
      exit(-1);
    }
    ...
    if (withNeighbors)
      net.haveFaceFromFace[NDIM][NDIM] = 1;
...
#pragma omp critical
  ...
  {
    double l1 = geometry->template distance2<Coord, NDIM_W>(seg1->getVertex(0)->getCoordsConstPtr(),
                                                            seg1->getVertex(1)->getCoordsConstPtr());
    double l2 = geometry->template distance2<Coord, NDIM_W>(seg2->getVertex(0)->getCoordsConstPtr(),
...
#pragma omp parallel for num_threads(nThreads) schedule(dynamic, 1)
    for (unsigned long i = 0; i < refinedSegments.size(); i++)
    {
      ...
      Vertex::Data::template onRefineVertex<MyType, SegmentHandle, Vertex, Simplex>(this, h, v, lst, nSimplices, buffer);
...
#pragma omp parallel for num_threads(nThreads) schedule(dynamic, 1)
    for (unsigned long i = 0; i < refinedSegments.size(); i++)
    {
      ...
      Simplex::Data::template onRefineSimplices<MyType, SegmentHandle, Vertex, Simplex>(this, h, v, lst, s, nSimplices, buffer);
  ...
  */
                                           ...
                                           bool skipShared = true)
  {
    ...
    for (unsigned long i = 0; i < newSimplices.size(); i++)
    {
      ...
      Simplex *other = newSimplices[i]->getNeighbor(newSimplices[i]->cache.c[0]);
...
#pragma omp critical
    ...
    for (long i = 0; i < vertexIndex.size(); ++i)
    {
      ...
      else
      {
        ...
        {
          PRINT_SRC_INFO(LOG_ERROR);
          glb::console->print<LOG_ERROR>("More than %ld new incident simplices for one vertex !", nNewMax);
          glb::console->print<LOG_ERROR>("This is highly suspicious, you may have a refinement problem ...");
          glb::console->print<LOG_ERROR>("You can also change the type of 'updtStatus' to something wider if you think you understand what you're doing.\n");
          exit(-1);
        }

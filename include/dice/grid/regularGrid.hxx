...
protected:
  ...
  void initSync()
  {
    ...
    for (i = 0; i < NDIM; i++)
    {
      ...
      if (grid->getLowMargin(i) > 0)
        marginDir[i].push_back(GridNav::dir(i, -1));
      if (grid->getHighMargin(i) > 0)
        marginDir[i].push_back(GridNav::dir(i, +1));
    }
    ...
    while (true)
    {
      ...
      if (direction)
      {
        ...
        receiveIterators.push_back(std::make_pair(local_fieldIterator(bit), local_fieldIterator(eit)));
      }
    ...
    for (i = 0; i < mpiCom->size(); i++)
    {
      if (i == mpiCom->rank())
      {
        ...
        for (j = 0; j < receiveDir.size(); j++)
        {
          ...
          for (k = 0; k < NDIM; k++)
            buffer[1 + (2 + NDIM) * j + 2 + k] = local_iterator::dim(it, k);
        }
      }
      ...
      for (j = 0; j < buffer[0]; j++)
      {
        if (buffer[1 + (2 + NDIM) * j] == mpiCom->rank())
        {
          ...
          for (k = 0; k < NDIM; k++)
            sendInfo[k] = buffer[1 + (2 + NDIM) * j + 2 + k];
          ...
          sendIterators.push_back(std::make_pair(local_fieldIterator(bit), local_fieldIterator(eit)));
        }
      }
    }
    ...
    if (glb::debug)
    {
      ...
      glb::console->print<LOG_DEBUG>("  (%d:) I send data to : %s.\n  (%d:) I receive data from: %s\n",
...
#pragma omp parallel for
  ...
  //! constructor
  explicit RegularGridT(const char *dataName_ = "value") : grid(NULL),
                                                           dataName(dataName_),
                                                           initialized(false)
  {
  ...
  RegularGridT(const MyType &other) : grid(NULL),
...
public:
  ...
  */
  ...
  void addAmrGrid(AMR *amr, int nThreads = glb::num_omp_threads, bool quiet = false)
  {
    ...
    sendCount[rk] = 0;
    if (!quiet)
      glb::console->printFlush<LOG_STD>("    * Painting local chunk ... ");
    timer.start();
    grid->addAmrGrid(amr, nThreads);
    double elapsed = timer.stop();
    if (!quiet)
      glb::console->printFlush<LOG_STD>("done in %.3gs\n", elapsed);

    if (sz == 1)
      return;

    if (!quiet)
      glb::console->printFlush<LOG_STD>("    * Preparing chunks for sending ... ");
    timer.start();
...
#pragma omp parallel for num_threads(nThreads)
    ...
    if (!quiet)
      glb::console->printFlush<LOG_STD>("    * Exchanging everything ... ");
    timer.start();
    ...
    if (!quiet)
      glb::console->printFlush<LOG_STD>("done in %.3gs\n", elapsed);
    if (!quiet)
      glb::console->printFlush<LOG_STD>("    * Painting remote chunks ... ");
    timer.start();
    for (int i = 0; i < sz; ++i)
    {
      ...
      if (receiveCount[i] > 0)
        grid->template addRemoteAmrGrid<AMR>(&receiveData[receiveDisp[i]], receiveCount[i], nThreads);
    }
  ...
  grid->template addRemoteAmrGrid<AMR>(receiveData[reqRk],nThreads);
    ...
    */
    elapsed = timer.stop();
    if (!quiet)
      glb::console->printFlush<LOG_STD>("done in %.2gs\n", elapsed);
                            ...
                            int whichField = -1)
  {
    ...
    for (int rk = 0; rk < sz; ++rk)
    {
      ...
      for (int j = 0; j < NDIM; ++j)
      {
        ...
        pos[rk][j] = remoteData[rk].params.position[j] + remoteData[rk].params.lowMargin[j];
        subDim[rk][j] = remoteData[rk].params.resolution[j] - remoteData[rk].params.lowMargin[j] - remoteData[rk].params.highMargin[j];
    ...
    long resolution[NDIM];
    for (int i = 0; i < NDIM; ++i)
      resolution[i] = lg.getResolution(i);
...
#pragma omp parallel for num_threads(nThreads)
    for (int th = 0; th < nThreads; ++th)
    {
      ...
      for (auto it = container.begin(th, nThreads); it != it_end; ++it)
      {
        ...
        ptr += kernel.template getCorePixelCoords<IS_PERIODIC, CellCenteredValues>(*it, lg.getOrigin(), lg.getBoxSizeInv(), iCoord);
  ...
  void gatherAll(LocalGrid &lg, const IT sendReceive[][2], bool alwaysInitializeGrid = false)
  {
    ...
    if (getNFields() < 2)
    {
      for (int i = 0; i < sz; ++i)
      {
        ...
        if (receiveCount[i] == 0)
          receiveType[i] = MPI_Type<Data>::get();
        else
        {

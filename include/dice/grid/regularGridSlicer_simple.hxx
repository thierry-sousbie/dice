 ...
 */
template <class G>
class RegularGridSlicerSimpleT : public internal::RegularGridSlicerBaseT<G>
{
public:
  ...
  */
  void slice(int index)
  {
    ...
    */
    ...
    {
      if (periodic)
        neighbors.push_back(Neighbor(0, 1, 0));
    }
    else
      neighbors.push_back(Neighbor(0, 1, myIndex + 1));

    if (slicePos[0] <= 0)
    {
      if (periodic)
        neighbors.push_back(Neighbor(0, -1, sliceCount[0] - 1));
    }
    else
      neighbors.push_back(Neighbor(0, -1, myIndex - 1));

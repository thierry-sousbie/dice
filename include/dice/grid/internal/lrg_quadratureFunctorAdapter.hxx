...
namespace internal
{
  namespace lrg
  {
    ...
    public:
      ...
      QFunctorAdapter(F &f, int fieldIndex) : functor(f)
      {
        fid = fieldIndex;
      }
      ...
      {
        return functor(fid, coords[0] + delta[0] * a, coords[1] + delta[1] * b);
      }

      double operator()(double a, double b, double c)
      {
        return functor(fid, coords[0] + delta[0] * a, coords[1] + delta[1] * b, coords[2] + delta[2] * c);
      }

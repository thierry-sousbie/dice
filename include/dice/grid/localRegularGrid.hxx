...
public:
  ...
  template <int order, int diff_order>
  friend struct internal::KernelDispacherT;
  ...
  glb::console->print<LOG_WARNING>
    ...
    */
    ...
    for (i = 0; i < NDIM; i++)
    {
      ...
      vertexCoord[i] = Scale::genScale(xMin[i], xMax[i], params.resolution[i], params.scale[i], ValLocationTypeV::VERTEX);
      cellCoord[i] = Scale::genScale(xMin[i], xMax[i], params.resolution[i], params.scale[i], ValLocationTypeV::CELL);
      valueCoord[i] = Scale::genScale(xMin[i], xMax[i], params.resolution[i], params.scale[i], params.valLocation[i]);

      if ((params.valLocation[i] == ValLocationTypeV::VERTEX) && (IS_PERIODIC))
        valueCoord[i].resize(valueCoord[i].size() - 1);

      vertexDelta[i] = Scale::genScaleDelta(xMin[i], xMax[i], params.resolution[i], params.scale[i], ValLocationTypeV::VERTEX);
      cellDelta[i] = Scale::genScaleDelta(xMin[i], xMax[i], params.resolution[i], params.scale[i], ValLocationTypeV::CELL);
    ...
    else
    {
      ...
      if (res != 0)
      {
        ...
        glb::console->print<LOG_ERROR>("Trying to allocate %ld bytes of 64 bytes aligned memory.\n", nAllocated * sizeof(value_type));
        glb::console->print<LOG_ERROR>("posix_memalign returned error %d.\n", res);
        exit(-1);
      }
    ...
    for (int k = 0; k < (1 << NDIM); k++)
    {
      for (i = 0; i < (1 << NDIM); i++)
      {
        ...
        if (j == NDIM)
          integrationPoints[k].push_back(ptr * nFields);
      }
    }
...
public:
  explicit LocalRegularGridT(const char *dataName_ = "value") : dataName(dataName_),
                                                                arr(NULL),
                                                                ownArr(false),
                                                                initialized(false)
  {
  ...
  long copyRawBuffer(Source &source)
  {
    ...
    // glb::console->print<LOG_WARNING>("nValues(%d) -> copy(%d).\n", nValues, count);
  ...
  LocalRegularGridT(const MyType &other) : dataName("copyConstructed"),
                                           arr(NULL),
                                           ownArr(false),
                                           initialized(false)
  {
...
public:
  ...
  iterator innerMargin_begin(const int *delta, Direction dir = GridNav::dir())
  {
    ...
    for (i = 0; i < NDIM; i++)
    {
      int d = delta[i];
      if (d == 0)
        d = valueCoord[i].size() - params.highMargin[i] - params.lowMargin[i];
      else if (d < 0)
        d = valueCoord[i].size();
  ...
  void interleave(const itT start, const itT stop, double fac = 1.)
  {
    ...
    {
      PRINT_SRC_INFO(LOG_WARNING);
      glb::console->print<LOG_WARNING>("nFields(%d) and number of grids(%ld) do not match.\n", nFields, std::distance(start, stop));
      exit(-1);
    }
...
#pragma omp parallel for num_threads(nThreads) // schedule(dynamic,ompDelta)
  ...
  */
  ...
  void interpolate(int order, CT *coords, double *result, int fieldIndex = -1) const
  {
    ...
    {
    case 0:
      internal::KernelDispacherT<0>::interpolate(*this, coords, result, fieldIndex);
      break;
    case 1:
      internal::KernelDispacherT<1>::interpolate(*this, coords, result, fieldIndex);
      break;
    case 2:
      internal::KernelDispacherT<2>::interpolate(*this, coords, result, fieldIndex);
      break;
    default:
      PRINT_SRC_INFO(LOG_ERROR);
  ...
  void interpolateGradient(int order, CT *coords, double *result, int fieldIndex = -1) const
  {
    ...
    case 1:
      internal::KernelDispacherT<1, 1>::interpolate(*this, coords, result, fieldIndex);
      break;
    case 2:
      internal::KernelDispacherT<2, 1>::interpolate(*this, coords, result, fieldIndex);
      break;
    default:
      PRINT_SRC_INFO(LOG_ERROR);
...
#pragma omp parallel num_threads(nThreads) reduction(+:result)
  ...
  */
  ...
  void applyKernel(K &kernel, CT *coords, double *result, int fieldIndex = -1) const
  {
    ...
    if (fieldIndex >= 0)
    {
      ...
      if (IS_INTERLEAVED)
        ...
        ptr += fieldIndex * getNValues();

      ptr += kernel.template setCoefs<IS_PERIODIC, CellCenteredValues>(coords, params.x0, boxSizeInv, pos);

      kernel.template apply<IS_PERIODIC, gridKernel::APPLY>(ptr, result, pos);
    }
    else
    {
      size_t delta = kernel.template setCoefs<IS_PERIODIC, CellCenteredValues>(coords, params.x0, boxSizeInv, pos);

      const Data *ptr = arr + delta;
      for (int i = 0; i < nFields; ++i)
      {
        kernel.template apply<IS_PERIODIC, gridKernel::APPLY>(ptr, result + i * K::OUT_COUNT, pos);
                     ...
                     int nThreads = glb::num_omp_threads)
  {
    ...
    default:
      PRINT_SRC_INFO(LOG_ERROR);
      glb::console->print<LOG_ERROR>("Unimplemented symmetry type : %s(=%ld) \n",

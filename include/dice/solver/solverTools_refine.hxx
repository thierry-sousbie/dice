...
namespace slv
{
  namespace refine
  {
            ...
            double threshold = 0, bool computeSegment = true)
    {
      ...
      double score = geometry->template distance2<Coord, M::NDIM_W>(ref->getVertex(0)->getCoordsConstPtr(),
                                                                    ref->getVertex(1)->getCoordsConstPtr());

      for (int i = 1; i < Simplex::NSEG; i++)
      {
        SegmentHandle seg = s->getSegmentHandle(i);
        double v = geometry->template distance2<Coord, M::NDIM_W>(seg->getVertex(0)->getCoordsConstPtr(),
                      ...
                      double threshold = 0, bool computeSegment = true)
    {
      ...
      int segIndex = 0;
      SegmentHandle ref = s->getSegmentHandle(segIndex);
      double score = geometry->template distance2<Coord, M::NDIM_W>(ref->getVertex(0)->initCoords.getPointer(),
                                                                    ref->getVertex(1)->initCoords.getPointer());

      for (int i = 1; i < Simplex::NSEG; i++)
      {
        SegmentHandle seg = s->getSegmentHandle(i);
        double v = geometry->template distance2<Coord, M::NDIM_W>(seg->getVertex(0)->initCoords.getPointer(),
                                    ...
                                    const typename M::GeometricProperties *geometry)
    {
      ...
      if (NDIM == 2)
      {
        result = fabs(internal::computeOrder2Invariant<NDIM, NDIM_W>(v[0], v[1], v[2],
      ...
      else if (NDIM == 3)
      {
        ...
        double tmp;
        tmp = fabs(internal::computeOrder2Invariant<NDIM, NDIM_W>(v[0], v[3], v[1],
        ...
        tmp = fabs(internal::computeOrder2Invariant<NDIM, NDIM_W>(v[0], v[2], v[3],
                                                                  t + NDIM_W * Simplex::template findSegmentIndex<0, 2>(),
                                                                  t + NDIM_W * Simplex::template findSegmentIndex<0, 3>(),
        ...
        tmp = fabs(internal::computeOrder2Invariant<NDIM, NDIM_W>(v[1], v[3], v[2],
                                           ...
                                           bool average = false)
    {
      ...
      if (average)
        return std::make_pair(avg / (2 * Simplex::NSEG), index);
      else
        return std::make_pair(result, index);
    }
                                   ...
                                   double threshold = 0, bool computeSegment = true)
    {
      ...
      {
        if ((M::BOUNDARY_TYPE != BoundaryType::PERIODIC) && (nb.neiV[k] == NULL))
          continue;
                            ...
                            const typename M::GeometricProperties *geometry)
    {
      ...
      for (int i = 0; i < Simplex::NVERT; ++i)
      {
        ...
        for (int j = 0; j < NDIM_W; ++j)
          barycenter[j] += geometry->checkCoordConsistency(coords[j], tracer[j], j);
      }
                                     ...
                                     bool average = false)
    {
      ...
      if (average)
        return std::make_pair(avg / (Simplex::NSEG /*-(excludeSeg!=-1)*/), index);
      else
        return std::make_pair(result, index);
    }
                ...
                double threshold = 0, bool computeSegment = true)
    {
      ...
      if (nb.nNeighbors != Simplex::NNEI)
        return CheckRefineReturnType(0, -1);
    ...
    template <class M>
    class CellDataFunctor_lengthT : public cellDataFunctors::InterfaceT<M, typename M::Simplex>
    {
    public:
      typedef cellDataFunctors::InterfaceT<M, typename M::Simplex> Base;
      typedef typename Base::Cell Cell;
      CellDataFunctor_lengthT(const M *m, int flags = cellDataFunctors::F_NO_FLAG) : Base(m, flags) {}
    ...
    template <class M>
    class CellDataFunctor_poincareInvariantT : public cellDataFunctors::InterfaceT<M, typename M::Simplex>
    {
    public:
      typedef cellDataFunctors::InterfaceT<M, typename M::Simplex> Base;
      typedef typename Base::Cell Cell;
      CellDataFunctor_poincareInvariantT(const M *m, int flags = cellDataFunctors::F_NO_FLAG) : Base(m, flags) {}

      double get(const Cell *c) const
      {
        // FIXME: const_cast => Design flaw
        // typename Cell::Neighborhood nb( const_cast<Cell*>(c) );
        return poincareInvariant<M>(const_cast<Cell *>(c), Base::mesh->getGeometry()).first;
      }
    ...
    template <class M>
    class CellDataFunctor_poincareInvariantWithSegTracersT : public cellDataFunctors::InterfaceT<M, typename M::Simplex>
    {
    public:
      typedef cellDataFunctors::InterfaceT<M, typename M::Simplex> Base;
      typedef typename Base::Cell Cell;
      CellDataFunctor_poincareInvariantWithSegTracersT(const M *m, int flags = cellDataFunctors::F_NO_FLAG) : Base(m, flags) {}

      double get(const Cell *c) const
      {
        // FIXME: const_cast => Design flaw
        // typename Cell::Neighborhood nb( const_cast<Cell*>(c) );
        return poincareInvariantWithSegTracers<M>(const_cast<Cell *>(c), Base::mesh->getGeometry());
      }
    ...
    template <class M>
    class CellDataFunctor_poincareInvariantWithSegTracers_order1T : public cellDataFunctors::InterfaceT<M, typename M::Simplex>
    {
    public:
      typedef cellDataFunctors::InterfaceT<M, typename M::Simplex> Base;
      typedef typename Base::Cell Cell;
      CellDataFunctor_poincareInvariantWithSegTracers_order1T(const M *m, int flags = cellDataFunctors::F_NO_FLAG) : Base(m, flags) {}

      double get(const Cell *c) const
      {
        // FIXME: const_cast => Design flaw
        // typename Cell::Neighborhood nb( const_cast<Cell*>(c) );
        return poincareInvariantWithSegTracers_order1<M>(const_cast<Cell *>(c), Base::mesh->getGeometry()).first;
      }
    ...
    template <class M>
    class CellDataFunctor_poincareInvariantFromNeighborsT : public cellDataFunctors::InterfaceT<M, typename M::Simplex>
    {
    public:
      typedef cellDataFunctors::InterfaceT<M, typename M::Simplex> Base;
      typedef typename Base::Cell Cell;
      CellDataFunctor_poincareInvariantFromNeighborsT(const M *m, int flags = cellDataFunctors::F_NO_FLAG) : Base(m, flags) {}

      double get(const Cell *c) const
      {
        // FIXME: const_cast => Design flaw
        typename Cell::Neighborhood nb(const_cast<Cell *>(c));
        return poincareInvariantFromNeighbors<M>(nb, Base::mesh->getGeometry(), 0, false).first;
      }
    ...
    template <class M>
    class CellDataFunctor_poincareInvariantBelowTracersT : public cellDataFunctors::InterfaceT<M, typename M::Simplex>
    {
    public:
      typedef cellDataFunctors::InterfaceT<M, typename M::Simplex> Base;
      typedef typename Base::Cell Cell;
      CellDataFunctor_poincareInvariantBelowTracersT(const M *m, int flags = cellDataFunctors::F_NO_FLAG) : Base(m, flags) {}

      double get(const Cell *c) const
      {
        // FIXME: const_cast => Design flaw
        // typename Cell::Neighborhood nb( const_cast<Cell*>(c) );
        return poincareInvariantBelowTracers<M>(const_cast<Cell *>(c), Base::mesh->getGeometry()).first;
      }
    ...
    template <class M>
    class CellDataFunctor_tracerDisplacementAngleT : public cellDataFunctors::InterfaceT<M, typename M::Simplex>
    {
    public:
      typedef cellDataFunctors::InterfaceT<M, typename M::Simplex> Base;
      typedef typename Base::Cell Cell;
      CellDataFunctor_tracerDisplacementAngleT(const M *m, int flags = cellDataFunctors::F_NO_FLAG) : Base(m, flags) {}

      double get(const Cell *c) const
      {
        // FIXME: const_cast => Design flaw
        // typename Cell::Neighborhood nb( const_cast<Cell*>(c) );
        return tracerDisplacementAngle<M>(const_cast<Cell *>(c), Base::mesh->getGeometry());
      }
    ...
    template <class M>
    class CellDataFunctor_volumeBelowT : public cellDataFunctors::InterfaceT<M, typename M::Simplex>
    {
    public:
      typedef cellDataFunctors::InterfaceT<M, typename M::Simplex> Base;
      typedef typename Base::Cell Cell;
      CellDataFunctor_volumeBelowT(const M *m, int flags = cellDataFunctors::F_NO_FLAG) : Base(m, flags) {}

...
namespace slv
{
  ...
  public:
    ...
    SolverInterfaceT(MpiCommunication *com = glb::mpiComWorld) : manager(glb::pParser) //, glb::console),
    ...
    SolverInterfaceT(const std::string &restartFileName,
                     MpiCommunication *com = glb::mpiComWorld) : manager(glb::pParser),
                                                                 mpiCom(com)
    {
    ...
    void construct()
    {
      ...
      BinaryReader::checkHeaderAndReport<LOG_ERROR, LOG_WARNING, Params>(glb::console, reader, paramsVersion, true);
      params.setSerializedVersion(paramsVersion);

      BinaryReader::checkHeaderAndReport<LOG_ERROR, LOG_WARNING, MeshParams>(glb::console, reader, meshParamsVersion, true);
      meshParams.setSerializedVersion(meshParamsVersion);

      BinaryReader::checkHeaderAndReport<LOG_ERROR, LOG_WARNING, SolverImplementation>(glb::console, reader, implVersion, true);

      implementation = new SolverImplementation(meshParams, params, reader, manager, implVersion, mpiCom);
...
#pragma omp parallel num_threads(glb::num_omp_threads)
    ...
    void initRestart()
    {
      ...
      BinaryReader::checkHeaderAndReport<LOG_ERROR, LOG_WARNING, MyType>(glb::console, reader, version, true);
    ...
    return;
      ...
      */
      ...
      if (!initialized)
      {
        ...
        if (timingsFile == NULL)
        {
          glb::console->print<LOG_ERROR>("Opening file %s for writing.\n",
      ...
      if (timingsFile == NULL)
      {
        glb::console->print<LOG_ERROR>("Opening file %s for appending.\n",
                                       timingsFileName.c_str());
        exit(-1);
      }
    ...
    if (timingsFile==NULL)
      ...
      */
      ...
      buildTimer->start();
      if (restarting)
        mesh->read(meshParams, reader);
      else
        implementation->onBuildMesh(mesh, meshParams);
      elapsed = buildTimer->check();
      glb::console->print<LOG_INFO>("Mesh was built in %lg seconds.\n", elapsed);

      glb::console->print<LOG_INFO>("Calling '%s' initializer.\n", SolverImplementation::classHeader().c_str());
      glb::console->indent();
      if (restarting)
        implementation->onRead(reader);
    ...
    SolverMainLoop:
           ...
           curTime += deltaT, curStep++)
      {
        ...
        updateTimer->start();
        implementation->onNewTimeStep(hlp::makeConst(curStep), hlp::makeConst(curTime), deltaT);
...
#pragma omp parallel num_threads(glb::num_omp_threads)
        ...
        if (glb::console->willPrint<LOG_PEDANTIC>())
        {
          ...
          glb::console->print<LOG_PEDANTIC>("The local mesh now has %ld(+%ldg/s) vertices and %ld(+%ldg/s) simplices\n",
                                            mesh->getNCells(0),

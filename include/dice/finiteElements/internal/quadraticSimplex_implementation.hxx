...
namespace internal
{
  namespace quadSimplex
  {
    ...
    template <int ND, int ORDER>
    struct GetIndexHelper;
    template <int ND, bool HIERACHICAL, int ORDER>
    struct ShapeFunctionsHelper;
    template <int ND, int NDW, bool HIERACHICAL, int ORDER>
    ...
    public:
      ...
      static void evalQuarterJacobian(const I in, const CM &cM, OM &out)
      {
        ...
        for (int dim = 0; dim < NDIM_W; ++dim)
          out.row(dim + 1) << cM(dim, 0) * 0.25 + cM(dim, 4) * in[1] + cM(dim, 5) * in[2] + cM(dim, 6) * in[3],
    ...
    public:
      ...
      static void evalQuarterJacobian(const I in, const CM &cM, OM &out)
      {
        out.row(0) << 0.25, 0.25, 0.25, 0.25;
        for (int dim = 0; dim < NDIM_W; ++dim)
          out.row(dim + 1) << cM(dim, 0) * (in[0] - 0.25) + cM(dim, 4) * in[1] + cM(dim, 5) * in[2] + cM(dim, 6) * in[3],
  ...
  for (int i=0;i< NDIM+1;++i)
    ...
    public:
      ...
      static void evalQuarterJacobian(const I in, const CM &coordsMat, OM &out)
      {
        ...
        for (int dim = 0; dim < NDIM_W; ++dim)
          out.row(dim + 1) << coordsMat(dim, 0) * 0.25 + coordsMat(dim, 3) * in[1] + coordsMat(dim, 4) * in[2],
    ...
    public:
      ...
      static void evalQuarterJacobian(const I in, const CM &coordsMat, OM &out)
      {
        out.row(0) << 0.25, 0.25, 0.25;
        for (int dim = 0; dim < NDIM_W; ++dim)
          out.row(dim + 1) << coordsMat(dim, 0) * (in[0] - 0.25) + coordsMat(dim, 3) * in[1] + coordsMat(dim, 4) * in[2],

...
class BarycentricCoordinatesT : protected internal::BarycentricCoordinatesBaseT<ND, CT>
{
public:
   ...
   */
  template <class T>
  BarycentricCoordinatesT(const T *const coords[ND + 1], const GP &geometry_) : geometry(geometry_)
  {
    // geometry=geometry_;
   ...
   */
  template <class T>
  BarycentricCoordinatesT(const T coords[ND + 1][ND], const GP &geometry_) : geometry(geometry_)
  {
    // geometry=geometry_;
    std::copy(coords[0], coords[0] + ND, origin);
   ...
   */
  template <class T>
  BarycentricCoordinatesT(const T *const coords[ND + 1]) : geometry(GP())
  {
    // geometry=geometry_;
    std::copy(coords[0], coords[0] + ND, origin);
   ...
   */
  template <class T>
  BarycentricCoordinatesT(const T coords[ND + 1][ND]) : geometry(GP())
  {
    // geometry=geometry_;
    std::copy(coords[0], coords[0] + ND, origin);
  ...
  */
  ...
  // std::numeric_limits<double>::epsilon()*10000.0F) const
  {
    ...
    for (int i = 0; i <= NDIM; ++i)
    {
      if (fabs(lambda[i]) <= tolerance)
        nZeroes++;
      else if (fabs(lambda[i] - 1) <= tolerance)
      {
        // nOnes++;
      }
      else if ((lambda[i] < 0) || (lambda[i] > 1))
        nOut++;
      else
        nIn++;
    }
    ...
    if (nIn <= NDIM)
    {
      if (nOut > 0)
        ...
        result = Location(1 << (NDIM + 1 - nZeroes));
    }

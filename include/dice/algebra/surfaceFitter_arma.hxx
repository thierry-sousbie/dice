...
public:
  ...
  SurfaceFitterT() : baseIsSet(false),
  ...
  FitFunctor fit(double &cond, double limitCondNumber = 1.E-3) const // bool debug=false) const
  {
    if (!baseIsSet)
    {
      ...
      glb::console->print<LOG_ERROR>("Tangent space base needs to be set before calling fit() !\n");
      exit(-1);
    }
    if ((!allowUnderDetermined) && (points.size() < NCOEFS))
    {
      PRINT_SRC_INFO(LOG_ERROR);
      glb::console->print<LOG_ERROR>("System is under-determined !\n");
      exit(-1);
    }
               ...
               double &cond, double limitCondNumber = -1.0, bool evalAngle = false)
  {
    ...
    if (evalAngle)
    {
      oldBase = base; /*oldBase.print("-> oldBase:");*/
    }

...
public:
  ...
  LocalAmrGridT(InputIterator x0, InputIterator delta) : //,MpiCommunication *mpiCom_=glb::mpiComWorld):
   ...
   */
  LocalAmrGridT(/*MpiCommunication *mpiCom_=glb::mpiComWorld*/) :                // rootVoxels(ROOT_VOXELS_COUNT),
                                                                  geometry(NULL) //,
  // mpiCom(mpiCom_)
  {
                                       ...
                                       bool force = false)
  {
    ...
    else
    {
      ...
      for (int i = 0; i < nThreads; ++i)
        visitors[i].init(this, assignSegments);
    ...
    glb::console->print<LOG_DEBUG>("Assigned %ld unique vertices and %ld segments.\n", nUniqueVertices, nUniqueSegments);
  ...
  */
                     ...
                     int nThreads = glb::num_omp_threads)
  {
    ...
    else
    {
      ...
      glb::console->print<LOG_WARNING>("Option 'fromScratch=false' ignored (not implemented yet !)\n");
      clear();
    }
...
#pragma omp parallel for num_threads(nThreads)
      for (int k = 0; k < nThreads; k++)
      {
        ...
        for (ghostSimplexPtr_iterator it =
                 mesh->ghostSimplexBegin(k, glb::num_omp_threads);
             it != itgs_end; ++it)
        {
          GhostSimplex *s = *it;
    ...
    */
    ...
    if ((maxLevel > 0) || (setSimplexCache))
    {
      ...
      for (; it != it_end; ++it)
      {
        ...
        double bBox[2][NDIM];
        bool bBoxIsConsistent = mesh->template computeBoundingBox<Simplex *, NDIM>(s, bBox);
        ...
        */
        ...
        if (M::BOUNDARY_TYPE != BoundaryType::PERIODIC)
        {
          for (int i = 0; i < Simplex::NNEI; ++i)
          {
            if (s->getNeighbor(i) == NULL)
            {
              mesh->template computeBoundingBox<FacetHandle, NDIM>(s->getFacetHandle(i), bBox);
              visitBBoxOverlap(bBox, visitor);
              // refineOverSimplex(mesh,s->getFacetHandle(i),maxLevel);
            }
          }
...
#pragma omp atomic
  ...
  */
  ...
  {
    int result = 0;
    if ((ref & INDEX_MASK) < (voxelIndex & INDEX_MASK))
      result++;
  ...
  void initialize(InputIterator x0, InputIterator delta) //, MpiCommunication *mpiCom_=NULL)
  {
    ...
    for (int i = 0; i < ND; ++i)
    {
      ...
      if (deltaX[i] > deltaX_max)
        deltaX_max = deltaX[i];
      if (deltaX[i] < deltaX_min)
        deltaX_min = deltaX[i];
...
*/
  ...
  static ICoord getNeighborIndex(ICoord index, int level, Direction dir)
  {
    ...
    if (BT != BoundaryType::PERIODIC)
    {
      ...
      index2 = (coord & (BBOX_ILEN - 1)); // periodic boundary
...
*/
  ...
  ICoord coords2Index(InputIterator coords, int level = MAX_LEVEL_FROM_ROOT) const
  {
    ...
    // else c=ICoord(((dc - xMin[0])*(deltaX_inv[0])) * BBOX_ILEN) | ICoord(1);
      ...
      c = hlp::numericStaticCast<ICoord>(((dc - xMin[0]) * (deltaX_inv[0])) * BBOX_ILEN_CPY) | ICoord(1);
    ...
    for (int i = 1; i < NDIM; ++i)
    {
      ...
      // else c=ICoord(((dc - xMin[i])*(deltaX_inv[i])) * BBOX_ILEN) | ICoord(1);
        ...
        c = hlp::numericStaticCast<ICoord>(((dc - xMin[i]) * (deltaX_inv[i])) * BBOX_ILEN_CPY) | ICoord(1);
      // else c=ICoord(((dc+halfEpsilon[i] - xMin[i])*(deltaX_inv[i])) * BBOX_ILEN) | ICoord(1);
      index |= (c << dec);
    }

    if (level != MAX_LEVEL_FROM_ROOT)
      index = setIndexLevel(index, level);
  ...
  ICoord coords2Index(InputIterator coords, T &warning, bool includeRightBoundary = true) const
  {
    ...
    for (int i = 1; i < NDIM; ++i)
    {
      ...
      {
        c = BBOX_ILEN - 1;
        if (!includeRightBoundary || (dc > xMax[i]))
          warning |= (1L << i);
      }
      else
      {
  ...
  */
                            ...
                            bool nonNullOnly = false)
  {
    internal::localAmrGridOverlapVisitor::
        GetLeavesBoxOverlapT<MyType, OutputIterator>
            visitor(out, nonNullOnly);

...
namespace internal {
  ...
  namespace localAmrGridOverlapVisitor {
    ...
    public:
      ...
      GetLeavesBoxOverlapT(OutputIterator out_, bool nonNullOnly_=false):
	out(out_),
	...
	if (voxel->getLevel()<level)
	  {
	    if (voxel->isLeaf()) voxel->refine(amr);
	    return true;
	  }
	else return false;
	/*
	...
	*/
    ...
    public:
      ...
      ThreadedRefineT(AMR *amr_, int level_):
	amr(amr_),
	level(level_)
      {}
	...
	if (voxel->getLevel()<level)
	  {
	    if (voxel->isLeaf()) voxel->refineCritical(amr);
	    return true;
	  }
	return false;
      }
    ...
    public:
      ...
      SetRootLevelT (AMR *amr_, int level_,const Voxel *rootVoxels,
		     std::vector<unsigned char> &rootVoxelsLevel):
	amr(amr_),
	level(level_),
	ref(rootVoxels),

...
namespace hlp {
  ...
  template <class T> struct FindFirstDifference<T,4> {
    static int find(T A[4],T B[4])
    {    
      if ((A[0]!=B[0])&&(A[0]!=B[1])&&(A[0]!=B[2])&&(A[0]!=B[3])) return 0;
      if ((A[1]!=B[0])&&(A[1]!=B[1])&&(A[1]!=B[2])&&(A[1]!=B[3])) return 1;
      if ((A[2]!=B[0])&&(A[2]!=B[1])&&(A[2]!=B[2])&&(A[2]!=B[3])) return 2;
      if ((A[3]!=B[0])&&(A[3]!=B[1])&&(A[3]!=B[2])&&(A[3]!=B[3])) return 3;
      return -1;
    }
  };
  template <class T> struct FindFirstDifference<T,3> {
    static int find(T A[3],T B[3])
    {    
      if ((A[0]!=B[0])&&(A[0]!=B[1])&&(A[0]!=B[2])) return 0;
      if ((A[1]!=B[0])&&(A[1]!=B[1])&&(A[1]!=B[2])) return 1;
      if ((A[2]!=B[0])&&(A[2]!=B[1])&&(A[2]!=B[2])) return 2;    
      return -1;
    }
  };  
  template <class T> struct FindFirstDifference<T,2> {
    static int find(T A[2],T B[2])
    {    
      if ((A[0]!=B[0])&&(A[0]!=B[1])) return 0;
      if ((A[1]!=B[0])&&(A[1]!=B[1])) return 1;      
      return -1;
    }
  };
  template <class T> struct FindFirstDifference<T,1> {
  ...
  struct FactorialT
  {
    enum {value = N*FactorialT<N-1>::value};
  };
  ...
  // T is a std::vector
  template<typename T>
  struct IsVector {typedef IsFalse Result;};
  template<typename T, typename A>
  struct IsVector<std::vector<T,A>> {typedef IsTrue Result;};
  template<typename T, typename A>
  struct IsVector<const std::vector<T,A>> {typedef IsTrue Result;};
  ...
  template <> struct IsPrimitiveType<bool> {typedef IsTrue Result;};
  template <> struct IsPrimitiveType<char> {typedef IsTrue Result;};
  template <> struct IsPrimitiveType<short> {typedef IsTrue Result;};
  template <> struct IsPrimitiveType<int> {typedef IsTrue Result;};
  template <> struct IsPrimitiveType<long> {typedef IsTrue Result;};
  template <> struct IsPrimitiveType<long long> {typedef IsTrue Result;};
  template <> struct IsPrimitiveType<unsigned char> {typedef IsTrue Result;};
  template <> struct IsPrimitiveType<unsigned short> {typedef IsTrue Result;};
  template <> struct IsPrimitiveType<unsigned int> {typedef IsTrue Result;};
  template <> struct IsPrimitiveType<unsigned long> {typedef IsTrue Result;};
  template <> struct IsPrimitiveType<unsigned long long> {typedef IsTrue Result;};
  template <> struct IsPrimitiveType<float> {typedef IsTrue Result;};
  template <> struct IsPrimitiveType<double> {typedef IsTrue Result;};
  template <> struct IsPrimitiveType<long double> {typedef IsTrue Result;}; 
  ...
  {    
    ...
    template<class T> struct IsBoostMultiExprTemplate {      
      // boost::multi expression templates define a U::result_type
      template<class U> static char (&test(typename U::result_type const*))[1];
  ...
  public:
    enum { result = ((sizeof(T)==sizeof(ConstantType<T>))&&(sizeof(Test<T>)==sizeof(Test< ConstantType<T> >))) };
  ...
  template <> struct MinimalIntegerType<0,false> : ConstantType<unsigned char> {};
  template <> struct MinimalIntegerType<0,true>  : ConstantType<char> {};
  template <> struct MinimalIntegerType<9,false> : ConstantType<unsigned short> {};
  template <> struct MinimalIntegerType<8,true>  : ConstantType<short> {};
  template <> struct MinimalIntegerType<17,false>: ConstantType<unsigned int> {};
  template <> struct MinimalIntegerType<16,true> : ConstantType<int> {};
  ...
  template <> struct MinimalIntegerType<33,false>: ConstantType<ULongInt64> {};
  template <> struct MinimalIntegerType<32,true> : ConstantType<LongInt64> {};
  
  // FIXME: define a 128bit integer type ??
  template <> struct MinimalIntegerType<65,false>: ConstantType<ULongInt128> {};
  template <> struct MinimalIntegerType<64,true> : ConstantType<LongInt128> {};

  // **** COUNT_BITS
  
  template<unsigned long long N> 
  struct CountBits : ConstantValue<1 + CountBits<(N>>1)>::value> {};
  template<> struct CountBits<0> : ConstantValue<0> {};
  template<> struct CountBits<1> : ConstantValue<1> {};
  ...
  struct OptimalSortT
  {   
    static void swap(T &a, T&b,IsEnabled) {if (a<b) std::swap(a,b);}
    static void swap(T &a, T&b,IsDisabled) {if (a>b) std::swap(a,b);}
  ...
  template <class T,class R> struct OptimalSortT<T,8,R>
  {   
    static void swap(T &a, T&b,IsEnabled) {if (a<b) std::swap(a,b);}
    static void swap(T &a, T&b,IsDisabled) {if (a>b) std::swap(a,b);}
    static void sort(T* arr)
    {    
      swap(arr[0],arr[1],R());swap(arr[2],arr[3],R());swap(arr[0],arr[2],R());
      swap(arr[1],arr[3],R());swap(arr[1],arr[2],R());swap(arr[4],arr[5],R());
      swap(arr[6],arr[7],R());swap(arr[4],arr[6],R());swap(arr[5],arr[7],R());
      swap(arr[5],arr[6],R());swap(arr[0],arr[4],R());swap(arr[1],arr[5],R());
      swap(arr[1],arr[4],R());swap(arr[2],arr[6],R());swap(arr[3],arr[7],R());
      swap(arr[3],arr[6],R());swap(arr[2],arr[4],R());swap(arr[3],arr[5],R());
      swap(arr[3],arr[4],R());
    }
  };
  template <class T,class R> struct OptimalSortT<T,7,R>
  {   
    static void swap(T &a, T&b,IsEnabled) {if (a<b) std::swap(a,b);}
    static void swap(T &a, T&b,IsDisabled) {if (a>b) std::swap(a,b);}
    static void sort(T* arr)
    {    
      swap(arr[1],arr[2],R());swap(arr[0],arr[2],R());swap(arr[0],arr[1],R());
      swap(arr[3],arr[4],R());swap(arr[5],arr[6],R());swap(arr[3],arr[5],R());
      swap(arr[4],arr[6],R());swap(arr[4],arr[5],R());swap(arr[0],arr[4],R());
      swap(arr[0],arr[3],R());swap(arr[1],arr[5],R());swap(arr[2],arr[6],R());
      swap(arr[2],arr[5],R());swap(arr[1],arr[3],R());swap(arr[2],arr[4],R());
      swap(arr[2],arr[3],R());
    }
  };
  template <class T,class R> struct OptimalSortT<T,6,R>
  {   
    static void swap(T &a, T&b,IsEnabled) {if (a<b) std::swap(a,b);}
    static void swap(T &a, T&b,IsDisabled) {if (a>b) std::swap(a,b);}
    static void sort(T* arr)
    {    
      swap(arr[1],arr[2],R());swap(arr[0],arr[2],R());swap(arr[0],arr[1],R());
      swap(arr[4],arr[5],R());swap(arr[3],arr[5],R());swap(arr[3],arr[4],R());
      swap(arr[0],arr[3],R());swap(arr[1],arr[4],R());swap(arr[2],arr[5],R());
      swap(arr[2],arr[4],R());swap(arr[1],arr[3],R());swap(arr[2],arr[3],R());
    }
  };
  template <class T,class R> struct OptimalSortT<T,5,R>
  {   
    static void swap(T &a, T&b,IsEnabled) {if (a<b) std::swap(a,b);}
    static void swap(T &a, T&b,IsDisabled) {if (a>b) std::swap(a,b);}
    static void sort(T* arr)
    {    
      swap(arr[0],arr[1],R());swap(arr[3],arr[4],R());swap(arr[2],arr[4],R());
      swap(arr[2],arr[3],R());swap(arr[0],arr[3],R());swap(arr[0],arr[2],R());
      swap(arr[1],arr[4],R());swap(arr[1],arr[3],R());swap(arr[1],arr[2],R());
    }
  };
  template <class T,class R> struct OptimalSortT<T,4,R>
  {   
    static void swap(T &a, T&b,IsEnabled) {if (a<b) std::swap(a,b);}
    static void swap(T &a, T&b,IsDisabled) {if (a>b) std::swap(a,b);}
    static void sort(T* arr)
    {    
      swap(arr[0],arr[1],R());swap(arr[2],arr[3],R());swap(arr[0],arr[2],R());
      swap(arr[1],arr[3],R());swap(arr[1],arr[2],R());
    }
  };
  template <class T,class R> struct OptimalSortT<T,3,R>
  {   
    static void swap(T &a, T&b,IsEnabled) {if (a<b) std::swap(a,b);}
    static void swap(T &a, T&b,IsDisabled) {if (a>b) std::swap(a,b);}
    static void sort(T* arr)
  ...
  template <class T,class R> struct OptimalSortT<T,2,R>
  {   
    static void swap(T &a, T&b,IsEnabled) {if (a<b) std::swap(a,b);}
    static void swap(T &a, T&b,IsDisabled) {if (a>b) std::swap(a,b);}
    static void sort(T* arr)
  ...
  template <class T,class R> struct OptimalSortT<T,1,R>
  { 
    static void swap(T &a, T&b,IsEnabled) {if (a<b) std::swap(a,b);}
    static void swap(T &a, T&b,IsDisabled) {if (a>b) std::swap(a,b);}
    static void sort(T* arr)
  ...
  // This is used to handles indices to iterate within a subarray
  ...
  */
  ...
  {
    typedef typename IsTrueT<SameType<T,double>::value
			     ||SameType<T,float>::value >::Result
      IsFloat;

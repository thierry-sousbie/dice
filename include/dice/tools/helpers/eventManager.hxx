...
public:
  ...
  void addEvents(std::string &cmd)
  {
    ...
    for (unsigned long i = 0; i < tokens.size(); ++i)
    {
      ...
      if ((tokens[i].at(0) == ':') || (std::isdigit(tokens[i].at(0))) || (tokens[i].at(0) == '-'))
      {
        ...
        if (t.size() == 1)
          addSingleEvent(val[0]);
        else if (t.size() == 2)
          addEvents(val[0], val[1]);
        else if (t.size() == 3)
          addEvents(val[0], val[1], val[2]);
        else
        {
  ...
  bool checkEventTimeFrame(double t0, double t1, double epsilon = 0) const
  {
    ...
    for (it = eventTriplets.begin(); it != eventTriplets.end(); ++it)
    {
      ...
      if (time0 != t0)
        time0 = (*it).start + ((*it).every) * (count0 + 1);
      if (time1 != t1)
        time1 = (*it).start + ((*it).every) * (count1 + 1);
  ...
  bool checkEvent(double curTime, double epsilon = 0) const
  {
    ...
    if (sit != events.end())
      if (fabs(*sit - curTime) <= epsilon)
        return true;

    if (b_sit != events.end())
      if (fabs(*b_sit - curTime) <= epsilon)
        return true;

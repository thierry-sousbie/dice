...
namespace internal {
  ...
  public:
    MpiCallTimerT(TimerPool::Timer *t, int th=0):
      ...
      thread(th)
    {
      if (omp_get_thread_num()==thread) timer->start();
    }

    ~MpiCallTimerT()
    {
      if (omp_get_thread_num()==thread) timer->stop();
    }
  private:
...
public:
  ...
  template <typename T, bool UseAsBarrier=false> T max(const T &val) const{return val;}

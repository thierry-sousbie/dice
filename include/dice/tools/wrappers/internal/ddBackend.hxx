...
namespace boost
{
      namespace multiprecision
      {
            namespace backends
            {
                  ...
                  public: // use private and friend for comparison operator
                        dd_real m_value;

                  public:
                        dd_backend() BOOST_NOEXCEPT : m_value(0) {}
                        dd_backend(const dd_backend &o) BOOST_NOEXCEPT : m_value(o.m_value) {}
                        dd_backend(double d) : m_value(d) {}
                        // dd_backend(int d) : m_value(static_cast<double>(d)) {}
                        ...
                        template <class T>
                        dd_backend(const T &i, const typename enable_if_c<std::is_convertible<T, dd_real>::value>::type * = 0) BOOST_NOEXCEPT
                            : m_value(i) {}
                        template <class T>
                        typename enable_if_c<is_arithmetic<T>::value || std::is_convertible<T, dd_real>::value, dd_backend &>::type operator=(const T &i) BOOST_NOEXCEPT
                        {
                              m_value = i;
                              return *this;
                        }
                  ...
                  {
                        return isnan(arg.value()) ? FP_NAN : isinf(arg.value()) ? FP_INFINITE
                                                         : arg.value() == 0     ? FP_ZERO
                                                                                : FP_NORMAL;
                  }
                  ...
                  */
                  ...
                  inline bool eval_lt(const dd_backend &a, double b) { return a.m_value < b; }
                  inline bool eval_gt(const dd_backend &a, double b) { return a.m_value > b; }
                  inline bool eval_eq(const dd_backend &a, double b) { return a.m_value == b; }

            } // namespace backends

            template <>
            struct number_category<backends::dd_backend> : public std::integral_constant<int, number_kind_floating_point>
            {
            };

            template <>
            struct number_category<dd_real> : public std::integral_constant<int, number_kind_floating_point>
            {
            };
            // template <>
            // struct number_category<backends::dd_backend>
            // {
            //       // Choose the appropriate kind for your backend:
            //       // number_kind_floating_point, number_kind_integer,
            //       // number_kind_rational, or number_kind_complex.
            //       static const static_number_category value = number_kind_floating_point;
            // };

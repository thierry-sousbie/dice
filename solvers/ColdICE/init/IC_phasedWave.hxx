...
template <class M>
class IC_phasedWave : public InitialConditionsInterfaceT<M>
{
private:
...
private:
  ...
  void parse(PM &paramsManager, R *reader)
  {
    ...
    aCross = 0.15;
    aCross = paramsManager.get("aCross", Interface::parserCategory(), aCross,
                               reader, PM::FILE_FIRST,
                               "Time of first shell crossing");

    kp = 1.0;
    kp = paramsManager.get("kp", Interface::parserCategory(), kp,
                           reader, PM::FILE_FIRST,
                           "Plane wave parameter");

    k[0] = 1.0;
    k[0] = paramsManager.get("k", Interface::parserCategory(), k[0], 0,
                             reader, PM::FILE_FIRST,
                             "Perturbation parameter");

    epsilon[0] = 0.5;
    epsilon[0] = paramsManager.get("epsilon", Interface::parserCategory(), epsilon[0], 0,
                                   reader, PM::FILE_FIRST,
                                   "");
    ...
    {
      k[i] = k[i - 1];
      k[i] = paramsManager.get("k", Interface::parserCategory(), k[i], i,
                               reader, PM::FILE_FIRST,
                               "Perturbation parameter");

      epsilon[i] = epsilon[i - 1] + 0.2;
      epsilon[i] = paramsManager.get("epsilon", Interface::parserCategory(), epsilon[i], i,
                                     reader, PM::FILE_FIRST,
                                     "");
    }
  }

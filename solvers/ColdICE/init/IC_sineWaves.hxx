...
template <class M>
class IC_sineWaves : public InitialConditionsInterfaceT<M>
{
private:
...
private:
  ...
  {
    cosmo = 1;
    cosmo = paramsManager.get("cosmo", Interface::parserCategory(), cosmo,
    ...
    amplitude[0] = paramsManager.get("amplitude", Interface::parserCategory(), amplitude[0], 0,
                                     reader, PM::FILE_FIRST,
    ...
    for (int i = 1; i < NDIM; ++i)
    {
      ...
      amplitude[i] = paramsManager.get("amplitude", Interface::parserCategory(), amplitude[i], i,
                                       reader, PM::FILE_FIRST,
                                       "Amplitude of the sine waves");
    }
    ...
    for (int k = 0; k < NDIM; ++k)
    {
      ...
      for (int i = 0; i < NDIM; ++i)
      {
        orientation[k][i] = paramsManager.get("orientation", Interface::parserCategory(), orientation[k][i], k * NDIM + i,
                                              reader, PM::FILE_FIRST,
        ...
        if ((orMin > orientation[k][i]) && (orientation[k][i] > 0))
          orMin = orientation[k][i];
      }
      ...
      for (int i = 2; i <= orMin; ++i)
      {
        ...
        if (isFactor)
        {
          ...
          orMin /= i;
          if (i > 1)
            i--; // account for factors multiplicity
        }
      }
    }
  ...
  for (int k=1;k<NDIM;k++)
    ...
    */
    ...
    dice::glb::console->printFlush<dice::LOG_INFO>("Initial conditions scale factor is f = (%lg, %lg).\n", scaleFactor[0], scaleFactor[1]);
    dice::glb::console->printFlush<dice::LOG_INFO>("Initial conditions scale factor is k = (%lg, %lg; %lg %lg).\n", kVec[0][0], kVec[0][1], kVec[1][0], kVec[1][1]);
    /*
    rescaleBox = 0;
  ...
  double dx2(double x[NDIM], int index)
  {
    ...
    for (int i = 0; i < NDIM; ++i)
      if (i != index)
        tmp += cos(2.0L * M_PI * (x[i] - 0.5)) * amplitude[i];
  ...
  void buildNextIVec(I iVec[NDIM][NDIM], int which)
  {
    ...
    else if (which == 1)
    {
      ...
      else
      {
        ...
        if (id.size() == 1)
          iVec[1][(id[0] + 1) % NDIM] = iVec[0][id[0]];
        else
        {

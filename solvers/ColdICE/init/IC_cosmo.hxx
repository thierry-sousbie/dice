...
template <class M>
class IC_cosmo : public InitialConditionsInterfaceT<M>
{
private:
                  ...
                  const Units &units, int pass)
  {
    if (pass == 0)
    {
      if (readHeaders(fname_) < 1)
      {
        ...
        dice::glb::console->print<dice::LOG_ERROR>("Invalid filename: '%s'\n", fname_.c_str());
...
protected:
  ...
  void parse(PM &paramsManager, R *reader)
  {
    ...
    fname_ = paramsManager.get("fileName", Interface::parserCategory(), fname_,
                               reader, PM::FILE_FIRST,
                               "Gagdet file name (without .N extension for multiple files).");

    maxMemPerNode_ = 1;
    maxMemPerNode_ = paramsManager.get("maxMemPerNode", Interface::parserCategory(), maxMemPerNode_,
                                       reader, PM::FILE_FIRST,
                                       "The maximum amount of memory to use on each MPI node, expressed in GigaBytes.");

    rowMajor_ = 1;
    rowMajor_ = paramsManager.get("rowMajor", Interface::parserCategory(), rowMajor_,
                                  reader, PM::FILE_FIRST,
    ...
    std::string initTesselationTypeStr = paramsManager.template get<std::string>("initTesselationType", Interface::parserCategory(),
                                                                                 dice::TesselationTypeSelect().getString(initTesselationType, true),
                                                                                 reader, PM::FILE_FIRST,
                                                                                 dice::TesselationTypeSelect().getAllString("Topology of the initial grid simplicial tesselation (%s)"));
    initTesselationType =
    ...
    for (int i = 0; i < NDIM; ++i)
    {
      ...
      x0_[i] = paramsManager.get("x0", Interface::parserCategory(), x0_[i], i,
                                 reader, PM::FILE_FIRST,
  ...
  {
    unsigned long index = stride_[0] * static_cast<unsigned long>((coords[0] - x0_[0] + epsilon_[0]) * deltaInv_[0]);
    for (int i = 1; i < NDIM; ++i)
      index += stride_[i] * static_cast<unsigned long>((coords[i] - x0_[i] + epsilon_[i]) * deltaInv_[i]);
...
private:
  ...
  long readHeaders(const std::string &name)
  {
    ...
    for (long i = 0; i < fullName.size(); ++i)
    {
      ...
      if (dummy != 256)
      {
        dice::glb::console->print<dice::LOG_WARNING>("File '%s' is an invalid gadget file !\n",
                                                     fullName[i].c_str());
        if (dummy == (256L << 24))
          dice::glb::console->print<dice::LOG_WARNING>("File and machine endianness differ.\n");
        return 0;
      }
    ...
    {
      dice::PRINT_SRC_INFO(dice::LOG_ERROR);
      dice::glb::console->print<dice::LOG_ERROR>("Inconsistent header: total npart = %ld, counted = %ld\n",
                                                 npartTotal, cum.back());
      throw std::runtime_error("Invalid filename");
    }
    ...
    while (iLen_ * iLen_ * iLen_ < cum.back())
      ++iLen_;
    if (iLen_ * iLen_ * iLen_ < cum.back())
    {
      dice::PRINT_SRC_INFO(dice::LOG_ERROR);
      dice::glb::console->print<dice::LOG_ERROR>("The number of particle should be a power of 3! (N=%ld)", cum.back());
      throw std::runtime_error("Invalid particles count filename");
    }

    stride_[0] = 1;
    for (int i = 1; i < NDIM; ++i)
      stride_[i] = stride_[i - 1] * iLen_;
  ...
  long readChunk(long start, long sz, double *ptr, long which)
  {
    ...
    while ((nLeft > 0) && (fIndex < headers.size()))
    {
      ...
      if (useFloats)
      {
        ...
        is.read((char *)ptr, nRead * floatSize * 3);
      ptr += nRead * 3;
      nLeft -= nRead;
      ++fIndex;
    }

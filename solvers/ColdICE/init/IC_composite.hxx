...
private:    
  ...
  void parse(PM &paramsManager, R *reader)
  {
    ...
    for (int i=0;i<NDIM_W;i++)
      {
	x0[i]=paramsManager.
	  get("bBoxX0",Interface::parserCategory(),x0[i],i,
	      reader,PM::FILE_FIRST,
	      "Initial coordinates of the bounding box lower left corner");
	
	delta[i]=paramsManager.
	  get("bBoxDelta",Interface::parserCategory(),delta[i],i,
	      reader,PM::FILE_FIRST,
    ...
    do {
      newComponent="none";
      newComponent=paramsManager.
	get("add",Interface::parserCategory(),newComponent,pass,
	    reader,PM::FILE_FIRST,
      ...
      if (newComponent!="none")
	{
	  ...
	  velVector[pass].val[0] = paramsManager.
	    get("velVector",Interface::parserCategory(),
	  ...
	  relativeDensity[pass] = paramsManager.
	    get("relativeDensity",Interface::parserCategory(),
	  ...
	  for (int i=1;i<NDIM;++i)
		...
		(pass>0)?velVector[pass-1].val[i]:velVector[pass].val[0];
	      velVector[pass].val[i] = paramsManager.
		get("velVector",Interface::parserCategory(),
	  ...
	  if (NDIM>2)
	    {
	      ...
	      angMomentumVec[pass].val[0] = paramsManager.
		get("angularMomentumVec",Interface::parserCategory(),
	      ...
	      for (int i=1;i<NDIM;++i)
		{
		  ...
		  angMomentumVec[pass].val[i] = paramsManager.
		    get("angularMomentumVec",Interface::parserCategory(),
	  ...
	  angMomentum[pass] = paramsManager.
	    get("angularMomentum",Interface::parserCategory(),

...
public:
                                                          ...
                                                          paramsManager, serializedVersion, mpiCom_)
  {
    ...
    std::string potentialTypeStr = paramsManager.get("potential", parserCategory(), PotentialTypeSelect().getString(potentialType, true), reader,
                                                     PM::PARSER_FIRST,
    ...
    gravitationalMass = paramsManager.get("gravitationalMass", parserCategory(), gravitationalMass, reader,
                                          PM::FILE_FIRST,
...
#pragma omp parallel for num_threads(dice::glb::num_omp_threads)
    for (int j = 0; j < dice::glb::num_omp_threads; j++)
    {
      ...
      for (MeshAndTracersCoords_iterator it =
               coordContainer.begin(j, dice::glb::num_omp_threads);
           it != it_end; ++it)
      {
        Coord *c = *it;
...
#pragma omp parallel num_threads(dice::glb::num_omp_threads)
  ...
  void advance_impl()
  {
    ...
    if (computePoisson)
      Base::solvePoisson(Base::curSolverTime + Base::curSolverDeltaT / 2);

    if ((dice::glb::debug) &&
        (Base::fileDumps.recheckEvent(FileDumps::Potential)))
    {
      Base::potential.template gatherSubsetAtCoords<typename Base::InterpolationKernel>(MeshAndTracersCoords(Base::mesh), Base::gatheredPotential);
      Base::gatheredPotential.toVtk((Base::fileDumps.getLocalFName(FileDumps::Potential) +

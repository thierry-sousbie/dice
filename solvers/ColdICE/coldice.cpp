...
int main(int argc, char **argv)
{
  ...
  int debugHook=glb::pParser->
    get<>("debugHook",dice::ParamsParser::defaultCategory(),0,
	  "Starts an infinite loop on startup to allow hooking a debugger");
  
  int debugWait=glb::pParser->
    get<>("debugWait",dice::ParamsParser::defaultCategory(),0,
	  "Wait for N seconds on startup to allow attaching a debugger");
  
  int help=glb::pParser->
    get<>("help",dice::ParamsParser::defaultCategory(),0); 
  
  int initOnly=glb::pParser->
    get<>("initOnly",dice::ParamsParser::defaultCategory(),0,
	  "Just initialize, report, and exit"); 
  
  int ignoreUnused=glb::pParser->
    get<>("ignoreUnusedParams",dice::ParamsParser::defaultCategory(),0,
	  "Do not exit when encountering unknown params");
  
  std::string restart=glb::pParser->
    get<>("restart",dice::ParamsParser::defaultCategory(),std::string(""),
	  "The name of a restart file  (without the '.rst' or '_${RANK}.rst'(MPI) extension)");  
  
  if ((initOnly)||(help)) glb::console->setVerboseLevel(3);

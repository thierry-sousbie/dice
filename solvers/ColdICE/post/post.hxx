...
public:
  ...
  ColdicePost(MeshParams &meshParams,
              ...
              dice::MpiCommunication *mpiCom_) : Base(meshParams, solverInterfaceParams, reader,
                                                      paramsManager, serializedVersion, mpiCom_)
  {
    typename PM::Parser *parser = paramsManager.getParser();

    gridLevel = Base::fftGridLevel;
    gridLevel = parser->get("gridLevel", parserCategory(), gridLevel,
                            "Sets the resolution of the density grid");

    projectionOrder = Base::projectionOrder;
    projectionOrder = parser->get("projectionOrder", parserCategory(), projectionOrder,
                                  "Order of the exact mass projection (0 or 1)");

    refineMesh = 0;
    refineMesh = parser->get("refineMesh", parserCategory(), refineMesh,
                             "Refine the mesh using tracers.");

    dumpAmr = 0;
    dumpAmr = parser->get("dumpAmr", parserCategory(), dumpAmr,
                          "Whether to dump AMR files.");

    dumpDensity = 0;
    dumpDensity = parser->get("dumpDensity", parserCategory(), dumpDensity,
                              "Whether to dump density grid files");

    dumpMesh = 0;
    dumpMesh = parser->get("dumpMesh", parserCategory(), dumpMesh,
                           "Whether to dump the unstructured mesh files");

    dumpSubmesh = 0;
    dumpSubmesh = parser->get("dumpSubmesh", parserCategory(), dumpSubmesh,
                              "Whether to dump the unstructured mesh subsets files");

    dumpRadialGridDensity = 0;
    dumpRadialGridDensity = parser->get("dumpRadialGridDensity", parserCategory(), dumpRadialGridDensity,
                                        "Whether to dump the radial grid density (from projected grid)");

    dumpRadialMeshDensity = 0;
    dumpRadialMeshDensity = parser->get("dumpRadialMeshDensity", parserCategory(), dumpRadialMeshDensity,
                                        "Whether to dump the radial mesh density (from the mesh)");

    radialMeshDensitySamples = 10;
    radialMeshDensitySamples = parser->get("radialMeshDensitySamples", parserCategory(), radialMeshDensitySamples,
  ...
  void onInitialize(Mesh *m, const SP &solverParams, double t, bool restart)
  {
    if (!(dumpMesh || dumpDensity || dumpAmr || dumpRadialGridDensity || dumpRadialMeshDensity))
    {
      ...
      dice::glb::console->printFlush<dice::LOG_STD>("At least one of the following options must be set :\n");
      dice::glb::console->printFlush<dice::LOG_STD>("    -post.dumpMesh -post.dumpAmr -post.dumpDensity.\n");
  ...
  double checkRefine_getValue(Simplex *s)
  {
    ...
    return dice::slv::refine::poincareInvariantWithSegTracers_order1<Mesh>(s, Base::geometry).first;
  }
...
protected:
  void process()
  {
    ...
    if (dumpMesh || dumpSubmesh)
      Base::dumpNetwork(dumpMesh, dumpSubmesh);

    if (dumpRadialMeshDensity)
      Base::dumpMeshDensityProfile(true, radialMeshDensitySamples);
    ...
    if (needProjectedDensity)
    {
      ...
      */
      ...
      if (dumpAmr)
        Base::localAmrDensity.toVtk(Base::fileDumps.getLocalFName(FileDumps::Amr).c_str());
      ...
      if (dumpDensity || dumpRadialGridDensity)
      {
        ...
        if (dumpDensity)
        {
          Base::potential.toVtk(Base::fileDumps.getGlobalFName(FileDumps::Density).c_str(),
...
private:
  ...
  void do_project()
  {
    if (projectionOrder == 0)
    {
      ...
      Base::localAmrDensity.template projectMesh0<Mesh, SimplexFunctor, checkAccuracy, IT, HT, ST>(Base::mesh, projectedDensityFunctor,
                                                                                                   Base::accuracyLevel,
    ...
    else
    {
      ...
      Base::localAmrDensity.template projectMesh1<Mesh, VertexFunctor, SimplexFunctor, checkAccuracy, IT, HT, ST>(Base::mesh, projectedDensityFunctor, projectedDensityGradientFunctor,
                                                                                                                  Base::accuracyLevel,
                                                                                                                  (Base::fastAmrBuild) ? true : false, // Use simplex tags
                                                                                                                  dice::glb::num_omp_threads,
                                                                                                                  true);

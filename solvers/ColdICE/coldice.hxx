...
public:
  ...
  Coldice(MeshParams &meshParams,
          ...
          dice::MpiCommunication *mpiCom_) : mesh(NULL),
                                             geometry(NULL),
                                             localAmrDensity(),
                                             ...
                                             repartStatus(false)
  {
    ...
    std::string icType =
        (serializedVersion < 0.145) ? std::string("uniformGrid") : std::string("sineWaves");

    std::stringstream icComment;
    icComment << "The type of initial condition: " << InitialConditionsFactory::getList();
    icType = paramsManager.get("type", "init", icType, reader,
                               PM::FILE_FIRST,
                               icComment.str(),
                               serializedVersion > 0.145);
    ic = InitialConditionsFactory::template create<Mesh>(icType, paramsManager, reader, mpiCom);

    if (ic == static_cast<IC *>(NULL))
    {
      dice::glb::console->printFlush<dice::LOG_ERROR>("Invalid initial conditions type: '%s'.\n", icType.c_str());
      dice::glb::console->print<dice::LOG_ERROR>("init.type must be one of the following: %s.\n",
    ...
    units.useCosmo = paramsManager.get("cosmo", parserCategory(), units.useCosmo, reader,
    ...
    if (units.useCosmo)
    {
      ...
      aStart = paramsManager.get("aStart", parserCategory(), aStart, reader,
                                 PM::FILE_FIRST, "Initial value of the scale factor");

      aEnd = paramsManager.get("aEnd", parserCategory(), aEnd, reader,
                               PM::PARSER_FIRST, "Final value of the scale factor");

      double dLogA = 0.1;
      dLogA = paramsManager.get("dLogA", parserCategory(), dLogA, reader,
      ...
      units.H = paramsManager.get("H", "units", units.H, reader,
                                  PM::FILE_FIRST,
    ...
    else
    {
      ...
      units.G = icp.G;
      units.G = paramsManager.get("G", "units", units.G, reader,
                                  PM::FILE_FIRST,
    ...
    units.length = paramsManager.get("length", "units", units.length, reader,
    ...
    units.velocity = paramsManager.get("velocity", "units", units.velocity, reader,
                                       PM::FILE_FIRST, comment);

    sprintf(comment, "Mass unit expressed in meters (default: %s).",
            icp.defaultUnitMass.c_str());
    units.mass = icp.unitMass;
    units.mass = paramsManager.get("mass", "units", units.mass, reader,
                                   PM::FILE_FIRST, comment);

    units.update();

    mass = icp.mass;
    mass = paramsManager.get("mass", parserCategory(), mass, reader,
                             PM::FILE_FIRST,
                             "Initial total mass");
    ...
    symmetryStr = paramsManager.get("symmetry", parserCategory(),
    ...
    std::string cflConditionStr = paramsManager.get("cflCondition", parserCategory(),
        ...
        (cflCondition == CflConditionTypeV::CFL_RHOMAX))
    {
      ...
      cflRhoMax = paramsManager.get("cflRhoMax", parserCategory(), cflRhoMax, reader,
                                    PM::PARSER_FIRST,
    ...
    {
      cflGrid = 0.25;
      cflGrid = paramsManager.get("cflGrid", parserCategory(), cflGrid, reader,
                                  PM::PARSER_FIRST,
    ...
    refineThreshold = 3.0;
    refineThreshold = paramsManager.get("refineThreshold", parserCategory(), refineThreshold, reader,
                                        PM::PARSER_FIRST,
                                        "NOT used");

    coarsenHysteresis = 0.95;
    coarsenHysteresis = paramsManager.get("coarsenHysteresis", parserCategory(), coarsenHysteresis, reader,
                                          PM::PARSER_FIRST,
                                          "NOT used");

    volumeThreshold = 1.E+10; // <=> desactivated
    volumeThreshold = paramsManager.get("volumeThreshold", parserCategory(), volumeThreshold, reader,
                                        PM::PARSER_FIRST,
                                        "Volume threshold to trigger refinement (NOT used)");

    invariantThreshold = 1.E-6;
    invariantThreshold = paramsManager.get("invariantThreshold", parserCategory(), invariantThreshold, reader,
                                           PM::PARSER_FIRST,
    ...
    splitLongestEdge = paramsManager.get("splitLongestEdge", parserCategory(), splitLongestEdge, reader,
                                         PM::PARSER_FIRST,
                                         "Set to split the longest longest edge when refining instead of trying to minimize the invariant");

    maxSimplexLevel = -1;
    maxSimplexLevel = paramsManager.get("maxSimplexLevel", parserCategory(), maxSimplexLevel, reader,
                                        PM::PARSER_FIRST,
    ...
    pUseVerticesThreshold = paramsManager.get("useVerticesThreshold", "projection", pUseVerticesThreshold, reader,
                                              PM::PARSER_FIRST,
    ...
    gatheredPotentialAllocLimit = paramsManager.get("gatheredPotentialAllocLimit", parserCategory(), gatheredPotentialAllocLimit, reader,
                                                    PM::PARSER_FIRST,
    ...
    fftGridLevel = paramsManager.get("fftGridLevel", parserCategory(), fftGridLevel, reader,
                                     PM::PARSER_FIRST,
    ...
    maxAmrLevel = paramsManager.get("maxAmrLevel", parserCategory(), maxAmrLevel, reader,
                                    PM::IGNORE_FILE,
                                    "Maximum refinement level of the AMR grid (don't touch that, it's already set ;) )");

    rebuildAmrEvery = 1;
    rebuildAmrEvery = paramsManager.get("rebuildAmrEvery", parserCategory(), rebuildAmrEvery, reader,
                                        PM::PARSER_FIRST,
    ...
    nThreads = paramsManager.get("nThreads", parserCategory(), nThreads, reader,
                                 PM::PARSER_FIRST,
    ...
    fastAmrBuild = 1;
    fastAmrBuild = paramsManager.get("fastAmrBuild", parserCategory(), fastAmrBuild, reader,
                                     PM::PARSER_FIRST,
    ...
    checkProjectedDensity = paramsManager.get("checkProjectedDensity", parserCategory(), checkProjectedDensity, reader,
                                              PM::PARSER_FIRST,
                                              "Check the positivity of the projected density (also look for NaN values).",
                                              serializedVersion > 0.105);

    phSortThreshold = 0.05;
    phSortThreshold = paramsManager.get("phSortThreshold", parserCategory(), phSortThreshold, reader,
                                        PM::PARSER_FIRST,
    ...
    fftWisdom = 1;
    fftWisdom = paramsManager.get("fftWisdom", parserCategory(), fftWisdom, reader,
                                  PM::PARSER_FIRST,
    ...
    projectionOrder = 1;
    projectionOrder = paramsManager.get("projectionOrder", parserCategory(), projectionOrder, reader,
                                        PM::PARSER_FIRST,
                                        "Order of the exact mass projection (only 0 and 1 are implemented so far)",
                                        serializedVersion > 0.115);

    exportFftWisdom = std::string("wisdom.fftw");
    exportFftWisdom = paramsManager.get("exportFftWisdom", parserCategory(), exportFftWisdom, reader,
                                        PM::PARSER_FIRST,
    ...
    importFftWisdom = paramsManager.get("importFftWisdom", parserCategory(), importFftWisdom, reader,
                                        PM::PARSER_FIRST,
                                        "The name of a file from which FFTW wisdom will be imported. Set to 'SKIP' or 'skip' to prevent importation.",
                                        serializedVersion > 0.135);

    noRepartWeight = 1;
    noRepartWeight = paramsManager.get("noRepartWeight", parserCategory(), noRepartWeight, reader,
                                       PM::PARSER_FIRST,
    ...
    accuracyLevel = 1.E-3;
    accuracyLevel = paramsManager.get("accuracyLevel", "projection", accuracyLevel, reader,
                                      PM::PARSER_FIRST,
                                      comment,
    ...
    */
    ...
    pEnableHRModeThreshold = paramsManager.get("enableHRModeThreshold", "projection", pEnableHRModeThreshold, reader,
                                               PM::PARSER_FIRST,
    ...
    pResetHRMode = 0;
    pEnableHRModeThreshold = paramsManager.get("resetHRMode", "projection", pResetHRMode, reader,
                                               PM::PARSER_FIRST,
    ...
    */
    ...
    pAnisotropyThreshold = paramsManager.get("anisotropyThreshold", "projection", pAnisotropyThreshold, reader,
                                             PM::PARSER_FIRST,
    ...
    pWidthThreshold = paramsManager.get("widthThreshold", "projection", pWidthThreshold, reader,
                                        PM::PARSER_FIRST,
    ...
    specifyProfileCenter = paramsManager.get("specifyProfileCenter", FileDumps::parserCategory(), specifyProfileCenter, reader,
                                             PM::PARSER_FIRST,
    ...
    for (int i = 0; i < NDIM; ++i)
    {
      ...
      densityProfileCenter[i] = paramsManager.get("densityProfileCenter", FileDumps::parserCategory(), densityProfileCenter[i], i,
...
#endif
    perSimplexInvariant = paramsManager.get("perSimplexInvariant", FileDumps::parserCategory(), perSimplexInvariant, reader,
                                            PM::PARSER_FIRST,
    ...
    {
      dice::glb::console->printFlush<dice::LOG_ERROR>("Cannot use a per simplex invariant threshold.\n");
      dice::glb::console->printFlush<dice::LOG_ERROR>("Unset 'perSimplexInvariant' or recompile with cmake option '-DPER_SIMPLEX_INVARIANT=true'\n");
      exit(-1);
    }
#endif
    ...
    skipInitialPoisson = parser->get("skipInitialPoisson", parserCategory(), skipInitialPoisson,
    ...
    dumpInitialMesh = 0;
    dumpInitialMesh = parser->get("dumpInitialMesh", parserCategory(), dumpInitialMesh,
  ...
  {
    return geometry->template distance2<Coord, NDIM_W>(v1->getCoordsConstPtr(), v2->getCoordsConstPtr()) <= coarsenThreshold2;
  }
  ...
  double checkRefine_getValue(Simplex *s)
  {
    ...
    if ((maxSimplexLevel < 0) || (s->getLevel() < maxSimplexLevel))
    {
      result = dice::slv::refine::
                   poincareInvariantWithSegTracers_order1<Mesh>(s, geometry)
                       .first;
  ...
  void onRead(R *reader)
  {
    ...
    if (pResetHRMode)
      pForceHighResolution = savedValue;
  ...
  void onWriteTimings(double t, OutputStream &out, bool writeHeader)
  {
    ...
    {
      if (units.useCosmo)
        out << " " << units.cosmology.a_of_tau(t, aStart, 1.E-8, aEnd * 2);
    }
  }
  ...
  void afterRepart(bool status)
  {
    ...
    else
    {
      ...
      // than the threshold
      dice::glb::console->printFlush<dice::LOG_STD>("Checking local mesh ordering ... ");
      double ratio = mpiCom->max(double(newMeshSimplicesCount) / mesh->getNSimplices());
      if (ratio > phSortThreshold)
      {
        dice::glb::console->print<dice::LOG_STD>("threshold crossed (f=%.2g).\n", ratio);
        sortMesh();
        ...
        dice::glb::console->print<dice::LOG_STD>("done. (f=%.2g < %.2g)\n", ratio, phSortThreshold);
    }
  }
...
protected:
                 ...
                 int forceNoFFTLevel = 0)
  {
    ...
    {
      dice::glb::console->printFlush<dice::LOG_STD>("Generating density grid using default slicer ... ");
    ...
    mesh->template addCellDataFunctor<dice::slv::refine::
                                          CellDataFunctor_poincareInvariantWithSegTracers_order1T>();
    mesh->template addCellDataFunctor<ProjectedDensityFunctorT>();
          ...
          (cflCondition == CflConditionTypeV::RHOMAX))
      {
        ...
        else
        {
          ...
          for (int th = 1; th < dice::glb::num_omp_threads; th++)
            if (result[th] > densityMax)
              densityMax = result[th];
  ...
  void initCellData(double t, bool resimulate = false)
  {
    ...
    dice::glb::console->printFlush<dice::LOG_STD>("Initializing cells (%s) in %ld pass(es) ... ", ic->getName().c_str(), nPasses);
...
#pragma omp atomic update
  ...
  void sortMesh()
  {
    ...
    dice::glb::console->print<dice::LOG_STD>("Sorting mesh cells (Peano-Hilbert):\n");
    ...
    dice::glb::console->print<dice::LOG_STD>("Done in %.2gs.\n", t);
  }
...
protected:
  ...
  {
    dice::glb::console->printFlush<dice::LOG_STD>("Building adaptive grid from mesh (LR:%d/%d) ... ", (int)D_AMR_ROOT_LEVEL, amrLevel);
    amrBuildTimer->start();
    ...
    dice::ProjectionTag projectionTag(pUseVerticesThreshold, dice::ProjectionTag::sampleFromVertices,
    ...
    if ((repartStatus) || ((rebuildAmrEvery - 1) <= nStepsSinceLastRebuild))
    {
      ...
      if (fastAmrBuild)
      {
        ...
        localAmrDensity.buildFromMesh_Fast(mesh, projectionTag, 1.0, level, true, dice::glb::num_omp_threads, true, NDIM > 2);
      }
      else
      {
        localAmrDensity.buildFromMesh(mesh, 1.0, level, true, dice::glb::num_omp_threads);
      }
    ...
    else
    {
      ...
      dice::glb::console->print<dice::LOG_ERROR>("rebuildAmrEvery must be 1 (NOT IMPLEMENTED YET).\n");
      exit(-1);
      /*
  ...
  if ((v->getLocalIndex()==81)||(v->getLocalIndex()==4049)||
    ...
    ****************/
    ...
    if (projectionOrder == 0)
    {
      ...
      reprojectedSimplicesCount = localAmrDensity.template projectMesh0<Mesh, SimplexFunctor,
                                                                        checkAccuracy, IT, HT, ST>(mesh, projectedDensityFunctor, accuracyLevel,
    ...
    else
    {
      ...
      reprojectedSimplicesCount = localAmrDensity.template projectMesh1<Mesh, VertexFunctor, SimplexFunctor,
                                                                        checkAccuracy, IT, HT, ST>(mesh, projectedDensityFunctor, projectedDensityGradientFunctor,
                                                                                                   accuracyLevel,
                                                                                                   (fastAmrBuild) ? true : false, // Use simplex tags
                                                                                                   dice::glb::num_omp_threads,
                                                                                                   true);
    ...
    if (mpiCom->size() > 1)
    {
      ...
      dice::glb::console->printFlush<dice::LOG_STD>("done in %.3lgs.\n", projectBarrierWaitDuration);
      dice::glb::console->printFlush<dice::LOG_STD>("Global projection imbalance factor: %.3lg (Tmin=%.3lgs, Tmax=%.3lgs, Tavg=%.3lgs).\n", imbalance, min, max, avg);
    }

    if (reprojectedSimplicesCount > pEnableHRModeThreshold * mesh->getGlobalNCells(NDIM))
    {
      dice::glb::console->print<dice::LOG_WARNING>("Reprojected simplices threshold reached, switching high resolution projection mode ON.\n");
    ...
    if (checkProjectedDensity)
    {
      ...
      // if (count>0)
      {
        ...
        if (count > 0)
          dice::glb::console->print<LOG>("Negative or NaN values found for the projected mass in %ld voxel(s).\n",
        ...
        if (result.size() > 0)
        {
          ...
          dice::glb::console->printFlush<LOG>("Negative voxels ID: V=%lg@%ld(%lg,%lg,%lg)",
                                              result[0]->data,
                                              result[0]->getIndex(),
          ...
          for (long i = 1; i < result.size(); ++i)
          {
            ...
            dice::glb::console->printFlush<LOG>(", V=%lg@%ld(%lg,%lg,%lg)",
                                                result[i]->data,
                                                result[i]->getIndex(),
                                                coords[0], coords[1], coords[2]);
          }
        ...
        {
          if (count > 0)
            dice::glb::console->print<LOG>("This most likely means that the floating point number type you are using for projection is not precise enough.\n");

          if (!dice::hlp::SameType<IT, HT>::value)
          {
            if (count > 0)
              dice::glb::console->print<LOG>("I Will try again using higher precision.\n");

            mpiCom->barrier();

            dice::glb::console->printFlush<dice::LOG_STD>("Retrying to project with high precision (expect up to a ~10 folds slowdown).\n");
            // Recompute density with higher resolution
            dice::glb::console->printFlush<dice::LOG_STD>("Updating projected density (HR) ... ");
        ...
        {
          if (count > 0)
            dice::glb::console->print<LOG>("This may either mean that you need extremely high precision, or that there is a subtle bug in the projection. Better luck next time ;(\n");
  ...
  void solvePoisson(double t)
  {
    ...
    if (units.useCosmo)
    {
      ...
      dice::glb::console->print<dice::LOG_STD>("Solving poisson equation:\n");
    ...
    // Compute statistics
    dice::glb::console->printFlush<dice::LOG_STD>("Computing mesh statistics ... ");
    ...
    mesh->quadratureFromList(quadratures, meshQuadratureResult, dice::glb::num_omp_threads, true);
    ...
    dice::glb::console->indent();
    dice::glb::console->print<dice::LOG_INFO>("-> Kinetic energy Ek = %e / %e\n", kineticEnergy, kineticEnergy_Q);
    dice::glb::console->print<dice::LOG_INFO>("-> Total volume Vt = %e / %e\n", totalVolume, totalVolume_Q);
    dice::glb::console->unIndent();
  ...
  if (dice::myIO::exists(fName))
    ...
    */
    ...
    if (fileDumps.checkEvent(FileDumps::Amr, true))
    {
      ...
      dice::glb::console->printFlush<dice::LOG_INFO>("File was dumped in %lgs.\n", elapsed);
    }
    ...
    {
      scatterDensityTimer->start();
      dice::glb::console->printFlush<dice::LOG_STD>("Enforcing '%s' symmetry on density field ... ", symmetryStr.c_str());
    ...
    if (dice::glb::debug > 2)
    {
      ...
      gatheredDensity.toVtk((fileDumps.getLocalFName(FileDumps::Potential) +
                             std::string("_gathered"))
                                .c_str());
    }
    ...
    dice::glb::console->indent();
    dice::glb::console->print<dice::LOG_INFO>("-> Potential energy Ep = %e\n", potentialEnergy);

    dice::glb::console->print<dice::LOG_INFO>("-> Projected mass pMass = %e (~%2.2e%% error)\n",
  ...
  void updateProjectedDensityField()
  {
    ...
    FOREACH_THREAD_SIMPLEX_LG(mesh, nThreads, th, it)
    {
      ...
      for (; it != it_end; ++it)
      {
        ...
        PDType volume = dice::hlp::numericStaticCast<PDType>(mesh->template computeProjectedVolume<Simplex, CT>(s));
...
#pragma omp parallel num_threads(nThreads)
  ...
  double getVelocityMax()
  {
    ...
    FOREACH_THREAD_VERTEX(mesh, dice::glb::num_omp_threads, th, it)
    {
      for (; it != it_end; ++it)
      {
        ...
        for (int i = 0; i < NDIM; ++i)
          if (fabs(c[i + NDIM]) > tmpMax[th])
            tmpMax[th] = fabs(c[i + NDIM]);
      }
    }
    FOREACH_THREAD_END;
  ...
  void updateInvariantThreshold(double t)
  {
    ...
    dice::glb::console->print<dice::LOG_STD>("Updated invariant Threshold (th=%g): %g\n",
  ...
  void updateTimeStep(double t, double &dt)
  {
    ...
    if (cflCondition == CflConditionTypeV::CFL_RHOMAX)
    {
      ...
      dt = mpiCom->min(dt);
      dice::glb::console->print<dice::LOG_STD>("Updated time step (dtMax=%g): dt=%g->%g (rhoMax=%e,dt=%g)/(vMax=%e,dt=%g)\n",
    ...
    else if (cflCondition == CflConditionTypeV::RHOMAX)
    {
      ...
      dt = mpiCom->min(dt);
      dice::glb::console->print<dice::LOG_STD>("Updated time step (dtMax=%g): dt=%g->%g (rhoMax=%e)\n",
    ...
    else if (cflCondition == CflConditionTypeV::CFL)
    {
      ...
      dt = mpiCom->min(dt);
      dice::glb::console->print<dice::LOG_STD>("Updated time step (dtMax=%g): dt=%g->%g (vMax=%e)\n",
                                               upperLimit, oldDt, dt, velocityMax);
    }

    if (units.useCosmo)
    {
      dice::glb::console->print<dice::LOG_STD>("New expansion factor is a=%g / z=%g (da=%g)\n",
...
#pragma omp parallel num_threads(nThreads)
    {
      ...
      for (MeshAndTracersCoords_iterator it =
               coordContainer.begin(th, nThreads);
           it != it_end; ++it)
      {
        Coord *c = *it;
...
#pragma omp parallel num_threads(nThreads)
    {
           ...
           it != its_end; ++it)
      {
        ...
        DICE_ALIGNAS(32)
        Coord temp1[nTracers * NDIM];
        DICE_ALIGNAS(32)
        Coord temp2[nTracers * NDIM];
        int k = 0;
  ...
  void advance_impl()
  {
    ...
    if (MeshAndTracersSliceCoordsT<Mesh>::getSlicesCount(potential, maxAlloc) > 1)
    {
      ...
      do
      {
        ...
        potential.template gatherSubsetAtCoords<InterpolationKernel>(coordContainer, gatheredPotential);
        ...
        dice::glb::console->print<dice::LOG_STD>("done in %.3gs / %.3gs. (%d/%d required)\n",
    ...
    else
    {
      ...
      gatherPotentialTimer->start();
      potential.template gatherSubsetAtCoords<InterpolationKernel>(MeshAndTracersCoords(mesh), gatheredPotential);
      ...
      {
        gatheredPotential.toVtk((fileDumps.getLocalFName(FileDumps::Potential) +
                                 std::string("_atCoords"))
                                    .c_str());
      }
...
protected:
  void dumpGridDensityProfile(bool force = false)
  {
    ...
    {
      statisticsTimer->start();
      dice::glb::console->printFlush<dice::LOG_STD>("Computing grid density profile ... ");

      RadialProfileGT<RegularGrid> profileG(&potential);
      if (specifyProfileCenter)
        profileG.setCenterCoordinates(densityProfileCenter);
      potential.visit(profileG);
      double elapsed = statisticsTimer->stop();
      dice::glb::console->printFlush<dice::LOG_INFO>("done in %lgs.\n", elapsed);

      dumpTimer->start();
      dice::glb::console->printFlush<dice::LOG_STD>("Dumping grid density profile to file '%s' ... ",
                                                    fileDumps.getGlobalFName(FileDumps::RadialGridDensity).c_str());

      char comment[255];
      sprintf(comment, "t=%lg step=%ld", curSolverTime, curStepIndex);
      profileG.toFile(fileDumps.getGlobalFName(FileDumps::RadialGridDensity), mpiCom, comment);
  ...
  void dumpMeshDensityProfile(bool force = false, int nSamplesPerOvelap = 10)
  {
    ...
    {
      statisticsTimer->start();
      dice::glb::console->printFlush<dice::LOG_STD>("Computing mesh density profile ... ");
      ...
      if (specifyProfileCenter)
        profileM.setCenterCoordinates(densityProfileCenter);
      ...
      char comment[255];
      sprintf(comment, "t=%lg step=%ld", curSolverTime, curStepIndex);
      profileM.toFile(fileDumps.getGlobalFName(FileDumps::RadialMeshDensity), mpiCom, comment);
      elapsed = dumpTimer->stop();
      dice::glb::console->printFlush<dice::LOG_INFO>("done.\n");
    }
  }
  ...
  void dumpNetwork(bool forceFull = false, bool forceSubNetwork = false)
  {
    ...
    if (dumped)
      dice::glb::console->printFlush<dice::LOG_INFO>("Files were dumped in %lgs.\n", elapsed);
  }
  ...
  void dumpCaustics()
  {
    ...
    mesh->template dumpToFilteredNDNetwork<decltype(causticsFilter), FacetHandle>(fileDumps.getLocalFName(FileDumps::Caustics).c_str(),
  ...
  void dumpSubsets()
  {
    ...
    mesh->template dumpToFilteredNDNetwork<decltype(subsetsFilter), Simplex>(fileDumps.getLocalFName(FileDumps::Subsets).c_str(),
  ...
  void dumpLagrangianLines()
  {
    ...
    mesh->template dumpToFilteredNDNetwork<LinesFilter, LinesFilterHandle>(fileDumps.getLocalFName(FileDumps::Lines).c_str(),

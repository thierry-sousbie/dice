...
class LessOrNanT : public std::unary_function<T, bool>
{
public:
...
template <class M>
class ProjectedDensityFunctorT : public dice::cellDataFunctors::InterfaceT<M, typename M::Simplex>
{
public:
  typedef dice::cellDataFunctors::InterfaceT<M, typename M::Simplex> Base;
  typedef typename Base::Cell Cell;
  ProjectedDensityFunctorT(const M *m, int flags = dice::cellDataFunctors::F_NO_FLAG) : Base(m, flags) {}
  double get(const Cell *c) const
  {
    double result = Base::mesh->template computeProjectedVolume<Cell, long double>(c);
    if (result != 0)
      result = c->mass.getValue() / result; // 1.0L
...
// vertices)
template <class M>
class SmoothedProjectedDensityFunctorT : public dice::cellDataFunctors::InterfaceT<M, typename M::Simplex>
{
public:
  typedef dice::cellDataFunctors::InterfaceT<M, typename M::Simplex> Base;
  typedef typename Base::Cell Cell;
  ...
  SmoothedProjectedDensityFunctorT(const M *m, int flags = dice::cellDataFunctors::F_NO_FLAG) : Base(m, flags) {}
  double get(const Cell *c) const
  {
...
public:
  ...
  void operator()(Simplex *s, int th) const
  {
    ...
    if ((anisotropy.size() < N) || (anisotropy.begin()->first < val))
    {
      ...
      if (anisotropy.size() > N)
        anisotropy.erase(anisotropy.begin());
    }
  ...
  void print()
  {
    ...
    minHeight.resize(1);
    volume.resize(1);
    while (anisotropy[0].size() > N)
    {
      anisotropy[0].erase(anisotropy[0].begin());
    }
    /*
    ...
    */
    ...
    std::cout << std::endl
              << "Anisotropy (minHeight/Volume):" << std::endl;
    int count = 0;
    ...
    std::cout << std::endl
              << "MinHeight (anisotropy/volume):" << std::endl;
    count = 0;
    ...
    std::cout << std::endl
              << "Volume (anisotropy/minHeight):" << std::endl;
    count = 0;
